...
#ifdef WITH_MPI
            call MPI_Irecv(bottom_border_recv_buffer(1,i), b_len,    &
#if REALCASE == 1
                           MPI_REAL_PRECISION,     &
#endif
#if COMPLEXCASE == 1
...
#ifdef WITH_MPI
           if (wantDebug) call obj%timer%start("mpi_communication")
           call mpi_bcast(bcast_buffer, nbw*current_local_n,    &
#if REALCASE == 1
                          MPI_REAL_PRECISION,    &
#endif
#if COMPLEXCASE == 1
...
#ifdef WITH_MPI
             if (wantDebug) call obj%timer%start("mpi_communication")
             call MPI_Isend(bottom_border_send_buffer(1,i), b_len,   &
#if REALCASE == 1
                            MPI_REAL_PRECISION,     &
#endif
#if COMPLEXCASE == 1
...
#ifdef WITH_MPI
        if (wantDebug) call obj%timer%start("mpi_communication")
        call MPI_Irecv(top_border_recv_buffer(1,i), b_len,     &
#if REALCASE == 1
                 MPI_REAL_PRECISION,    &
#endif
#if COMPLEXCASE == 1
...
!               bottom_border_send_buffer(1:stripe_width,1:next_top_msg_length,i)
...
#ifdef WITH_MPI
        if (wantDebug) call obj%timer%start("mpi_communication")
        call MPI_Isend(top_border_send_buffer(1,i), b_len,     &
#if REALCASE == 1
                 MPI_REAL_PRECISION, &
#endif
#if COMPLEXCASE == 1
...
#ifdef WITH_MPI
        if (wantDebug) call obj%timer%start("mpi_communication")
        call MPI_Isend(top_border_send_buffer(1,1,i), nbw*stripe_width,     &
#if REALCASE == 1
                 MPI_REAL_PRECISION,    &
#endif
#if COMPLEXCASE == 1
    ...
    if (my_prow == 0) then
...
#ifdef WITH_MPI
          if (wantDebug) call obj%timer%start("mpi_communication")
          call MPI_Isend(result_buffer(1,1,nbuf), l_nev*nblk,    &
#if REALCASE == 1
                   MPI_REAL_PRECISION,       &
#endif
#if COMPLEXCASE == 1
      ...
      do j = num_bufs_recvd, num_result_blocks-1
        ...
        if (j+num_result_buffers < num_result_blocks) &
            call MPI_Irecv(result_buffer(1,1,nbuf), l_nev*nblk, &
#if REALCASE == 1
                     MPI_REAL_PRECISION,   &
#endif
#if COMPLEXCASE == 1
