...
!                                   mpi_comm_rows, mpi_comm_cols)
  ...
  instance1 = elpa_create(na, nev, na_rows, na_cols, nblk)


  ! query some information

  success_test = instance1%get_option("use_qr", use_qr)
  print *,"At the moment use_qr is set to: ",use_qr
  success_test = instance1%get_option("solver",solver)
  print *,"At the moment solver is set to: ",trim(solver)

  ! set some options
  success_test = instance1%set_option("solver","2stage")
  success_test = instance1%set_option("real_kernel",REAL_ELPA_KERNEL_GENERIC)
  success_test = instance1%set_option("timings","balanced")
  success_test = instance1%set_option("use_qr",0)
  success_test = instance1%set_option("use_gpu",0)


   mpierr = instance1%get_communicators(mpi_comm_world, my_prow, my_pcol, &
                                   mpi_comm_rows, mpi_comm_cols)
...
#endif
   success_test = instance1%solve_real_double(a, ev, z)
