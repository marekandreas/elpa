...
#endif
         if (l_row1>1 .and. l_cols-l_col1+1>0) &
           call ZGEMM('N','N',l_row1-1,l_cols-l_col1+1,nb, (-1._rk,0._rk), &
                        tmat1,ubound(tmat1,dim=1),tmat2(1,l_col1),ubound(tmat2,dim=1), &
                        (1._rk,0._rk), a(1,l_col1),lda)
        ...
        do nb=0,(l_rows_np-1)/nblk
          ...
          if (nstor==nblk_mult .or. nb*nblk+nblk >= l_rows_np) then
            ...
            if (lcs<=lce) then
              ...
              if (lrs<=lre) then
                call zgemm('C','N',nstor,lce-lcs+1,lre-lrs+1,(1._rk,0._rk),aux_mat(lrs,1),ubound(aux_mat,dim=1), &
                             b(lrs,lcs),ldb,(0._rk,0._rk),tmp1,nstor)
               else
                 tmp1 = 0
               endif
