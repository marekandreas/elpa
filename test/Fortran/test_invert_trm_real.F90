...
program test_invert_trm
  ...
  call timer%start("program")
   ...
   call prepare_matrix(na, myid, sc_desc, iseed,  b, z, bs)
   bs(:,:) = b(:,:)

   a(:,:) = 0.0_rk


   diagonalElement = 2.546_rk
   ...
   success = elpa_cholesky_real(na, a, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, .true.)
   ...
   success = elpa_invert_trm_real(na, a, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, .true.)
...
#ifdef WITH_MPI
   call pdgemm("N","N", na, na, na, 1.0_rk, as, 1, 1, sc_desc, a, 1, 1, &
               sc_desc, 0.0_rk, tmp1, 1, 1, sc_desc)
#else
   call dgemm("N","N", na, na, na, 1.0_rk, as, na, a, na, 0.0_rk, tmp1, na)
#endif
   ...
   ! tmp2 = b * tmp1
#ifdef WITH_MPI
   call pdgemm("N","N", na, na, na, 1.0_rk, b, 1, 1, sc_desc, tmp1, 1, 1, &
               sc_desc, 0.0_rk, tmp2, 1, 1, sc_desc)
#else
   call dgemm("N","N", na, na, na, 1.0_rk, b, na, tmp1, na, 0.0_rk, tmp2, na)
#endif
