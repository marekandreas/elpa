...
use cublas
   ...
   real*8 a(lda,na), d(na), e(na), tau(na)
...
!$ integer omp_get_thread_num, omp_get_num_threads, omp_get_max_threads
   ...
   real*8, allocatable, pinned:: tmp(:), vr(:), vc(:), ur(:), uc(:), vur(:,:), uvc(:,:)
   !real*8, allocatable:: tmp(:), vr(:), vc(:), ur(:), uc(:), vur(:,:), uvc(:,:)
   real*8, allocatable:: ur_p(:,:), uc_p(:,:)

   real*8, allocatable, device :: a_dev(:,:), vr_dev(:), ur_dev(:), vc_dev(:), uc_dev(:), vur_dev(:,:), uvc_dev(:,:)
   !real*8, allocatable :: a_dev(:,:), vr_dev(:), ur_dev(:), vc_dev(:), uc_dev(:), vur_dev(:,:), uvc_dev(:,:)
real*8 times(100), ttt0, ttt1, ttts
...
ttt0 = mpi_wtime()
   ...
   tile_size = ((512*max(np_rows,np_cols)-1)/tile_size+1)*tile_size
...
times(11) = times(11)+mpi_wtime()-ttt1
ttt1 = mpi_wtime()
...
times(12) = times(12)+mpi_wtime()-ttt1
!$OMP END PARALLEL
ttt1 = mpi_wtime()
...
!print *,'done copy'
times(13) = times(13)+mpi_wtime()-ttt1
...
times(14) = times(14)+mpi_wtime()-ttt1
...
print '("Times: ",10f10.3)',times(11:14)
...
use cublas
   ...
   real*8 a(lda,*), q(ldq,nqc), tau(na)

   integer :: max_stored_rows

   integer my_prow, my_pcol, np_rows, np_cols, mpierr
   integer totalblocks, max_blocks_row, max_blocks_col, max_local_rows, max_local_cols
   integer l_cols, l_rows, l_colh, nstor, hvm_ubnd
   integer istep, i, n, nc, ic, ics, ice, nb, cur_pcol

   real*8, allocatable:: tmp1(:), tmp2(:), hvb(:), hvm(:,:), hvm1(:)
   real*8, allocatable:: tmat(:,:), h1(:), h2(:)

   real*8, allocatable, device :: q_dev(:,:), tmp_dev(:), hvm_dev(:), tmat_dev(:,:)
   !real*8, allocatable :: q_dev(:,:), tmp_dev(:), hvm_dev(:), tmat_dev(:,:)
   ...
   do istep=1,na,nblk
      ...
      if(nstor+nblk>max_stored_rows .or. istep+nblk>na .or. (na/np_rows<=256 .and. nstor>=32)) then
         ...
         if(l_rows>0) then
            call dgemm('T','N',nstor,l_cols,l_rows,1.d0,hvm_dev,hvm_ubnd, &
                       q_dev,ldq,0.d0,tmp_dev,nstor)
         else
            tmp_dev(1:l_cols*nstor) = 0
         endif
         ! call mpi_allreduce(tmp1,tmp2,nstor*l_cols,MPI_REAL8,MPI_SUM,mpi_comm_rows,mpierr)
         if(l_rows>0) then
            call dtrmm('L','L','N','N',nstor,l_cols,1.0d0,tmat_dev,max_stored_rows,tmp_dev,nstor)
            call dgemm('N','N',l_rows,l_cols,nstor,-1.d0,hvm_dev,hvm_ubnd, &
                       tmp_dev,nstor,1.d0,q_dev,ldq)
         endif
         nstor = 0
...
subroutine solve_tridi_col( na, nev, nqoff, d, e, q, ldq, nblk, mpi_comm_rows )
   ...
   n = na
   ndiv = 1
!!! We need more calls to merge_systems
  ...
  ! do while(n>512)
   ...
   if(np_rows==1 .and. nev<na .and. na>2*min_submatrix_size) ndiv = max(ndiv,2)
...
!      print*, 'Ndiv = ', ndiv
      do np = 0, ndiv-1
       ...
       !  print*, "Broadcasting 1 with root = ", np
         call MPI_Bcast(d(noff+1),nlen,MPI_REAL8,np,mpi_comm_rows,mpierr)
         qmat2 = qmat1
       !  print*, "Broadcasting 2 with root = ", np
...
recursive subroutine solve_tridi_single(nlen, d, e, q, ldq)
   ...
   integer i, j, lwork, liwork, info, mpierr, nm
...
!
!   q(1:nlen,1:nlen) = 0.
!   call solve_tridi_single(nm,d,e,q,ldq)
!   call solve_tridi_single(nlen-nm,d(nm+1),e(nm+1),q(nm+1,nm+1),ldq)
!   call merge_systems_single(nlen, nm, d, e(nm), q, ldq )
!
!else
...
use cublas
   ...
   real*8, allocatable :: qtmp1(:,:), ev_loc(:,:)
   real*8, allocatable, device :: qtmp1_dev(:,:), qtmp2(:,:), ev(:,:)
   !real*8, allocatable :: qtmp1_dev(:,:), qtmp2(:,:), ev(:,:)
...
!$OMP END PARALLEL DO
      ...
      allocate(ev_loc(max_local_cols,MIN(max_strip,MAX(1,nqcols1))))
...
ttt0 = mpi_wtime()
            do i = 1, ncnt
               ...
               ev_loc(1:nnzu,i) = zu(1:nnzu) / tmp(1:nnzu) * ev_scale(j)
            enddo
ev(:,1:ncnt) = ev_loc(:,1:ncnt)
ta = ta + mpi_wtime()-ttt0
...
ttt0 = mpi_wtime()
            do i = 1, ncnt
               ...
               ev_loc(1:nnzl,i) = zl(1:nnzl) / tmp(1:nnzl) * ev_scale(j)
            enddo
ev(:,1:ncnt) = ev_loc(:,1:ncnt)
ta = ta + mpi_wtime()-ttt0
...
!-------------------------------------------------------------------------------
   ...
   !print *, ((nblks_tot+lcm_s_t-1)/lcm_s_t) * nblk * nvc
