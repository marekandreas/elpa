...
extern "C" void hip_copy_double_tmp2_c_FromC(double *tmp2_dev, double *c_dev, int *nr_done_in, int *nstor_in, int *lcs_in, int *lce_in, int *ldc_in, int *ldcCols_in, hipStream_t my_stream) { 
  ...
  hip_copy_double_tmp2_c_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(tmp2_dev, c_dev, nr_done, nstor, lcs, lce, ldc, ldcCols);
#else
  hip_copy_double_tmp2_c_kernel<<<blocks,threadsPerBlock>>>(tmp2_dev, c_dev, nr_done, nstor, lcs, lce, ldc, ldcCols);
#endif
...
#ifdef WITH_GPU_STREAMS
  hip_copy_float_tmp2_c_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(tmp2_dev, c_dev, nr_done, nstor, lcs, lce, ldc, ldcCols);
#else
  hip_copy_float_tmp2_c_kernel<<<blocks,threadsPerBlock>>>(tmp2_dev, c_dev, nr_done, nstor, lcs, lce, ldc, ldcCols);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
#ifdef WITH_GPU_STREAMS
  hip_copy_double_complex_tmp2_c_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(tmp2_casted, c_casted, nr_done, nstor, lcs, lce, ldc, ldcCols);
#else
  hip_copy_double_complex_tmp2_c_kernel<<<blocks,threadsPerBlock>>>(tmp2_casted, c_casted, nr_done, nstor, lcs, lce, ldc, ldcCols);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
#ifdef WITH_GPU_STREAMS
  hip_copy_float_complex_tmp2_c_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(tmp2_casted, c_casted, nr_done, nstor, lcs, lce, ldc, ldcCols);
#else
  hip_copy_float_complex_tmp2_c_kernel<<<blocks,threadsPerBlock>>>(tmp2_casted, c_casted, nr_done, nstor, lcs, lce, ldc, ldcCols);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
extern "C" void hip_copy_double_a_aux_bc_FromC(double *a_dev, double *aux_bc_dev, int *n_aux_bc_in, int *nvals_in, int *lrs_in, int *lre_in, int *noff_in, int *nblk_in, int *n_in, int *l_rows_in, int *lda_in, int *ldaCols_in, hipStream_t my_stream) { 
...
#ifdef WITH_GPU_STREAMS
  hip_copy_double_a_aux_bc_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(a_dev, aux_bc_dev, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#else
  hip_copy_double_a_aux_bc_kernel<<<blocks,threadsPerBlock>>>(a_dev, aux_bc_dev, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
#ifdef WITH_GPU_STREAMS
  hip_copy_float_a_aux_bc_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(a_dev, aux_bc_dev, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#else
  hip_copy_float_a_aux_bc_kernel<<<blocks,threadsPerBlock>>>(a_dev, aux_bc_dev, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
extern "C" void hip_copy_double_complex_a_aux_bc_FromC(double _Complex *a_dev, double _Complex *aux_bc_dev, int *n_aux_bc_in, int *nvals_in, int *lrs_in, int *lre_in, int *noff_in, int *nblk_in, int *n_in, int *l_rows_in, int *lda_in, int *ldaCols_in, hipStream_t my_stream) { 
...
#ifdef WITH_GPU_STREAMS
  hip_copy_double_complex_a_aux_bc_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(a_dev_casted, aux_bc_dev_casted, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#else
  hip_copy_double_complex_a_aux_bc_kernel<<<blocks,threadsPerBlock>>>(a_dev_casted, aux_bc_dev_casted, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
extern "C" void hip_copy_float_complex_a_aux_bc_FromC(float _Complex *a_dev, float _Complex *aux_bc_dev, int *n_aux_bc_in, int *nvals_in, int *lrs_in, int *lre_in, int *noff_in, int *nblk_in, int *n_in, int *l_rows_in, int *lda_in, int *ldaCols_in, hipStream_t my_stream) { 
...
#ifdef WITH_GPU_STREAMS
  hip_copy_float_complex_a_aux_bc_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(a_dev_casted, aux_bc_dev_casted, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#else
  hip_copy_float_complex_a_aux_bc_kernel<<<blocks,threadsPerBlock>>>(a_dev_casted, aux_bc_dev_casted, n_aux_bc, nvals, lrs, lre, noff, nblk, n, l_rows, lda, ldaCols);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
extern "C" void hip_copy_double_aux_bc_aux_mat_FromC(double *aux_bc_dev, double *aux_mat_dev, int *lrs_in, int *lre_in, int *nstor_in, int *n_aux_bc_in, int *nvals_in, int *l_rows_in, int *nblk_in, int *nblk_mult_in , hipStream_t my_stream) {
...
#ifdef WITH_GPU_STREAMS
  hip_copy_double_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(aux_bc_dev, aux_mat_dev, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#else
  hip_copy_double_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock>>>(aux_bc_dev, aux_mat_dev, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
#ifdef WITH_GPU_STREAMS
  hip_copy_float_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(aux_bc_dev, aux_mat_dev, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#else
  hip_copy_float_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock>>>(aux_bc_dev, aux_mat_dev, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
#ifdef WITH_GPU_STREAMS
  hip_copy_double_complex_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(aux_bc_dev_casted, aux_mat_dev_casted, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#else
  hip_copy_double_complex_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock>>>(aux_bc_dev_casted, aux_mat_dev_casted, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
#ifdef WITH_GPU_STREAMS
  hip_copy_float_complex_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock, 0, my_stream>>>(aux_bc_dev_casted, aux_mat_dev_casted, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#else
  hip_copy_float_complex_aux_bc_aux_mat_kernel<<<blocks,threadsPerBlock>>>(aux_bc_dev_casted, aux_mat_dev_casted, lrs, lre, nstor, n_aux_bc, nvals, l_rows, nblk, nblk_mult);
#endif
  hipError_t cuerr = hipGetLastError();
  if (cuerr != hipSuccess){
...
template <typename T>
__global__ void hip_copy_aux_full_kernel(T *lhs_dev, T *rhs_dev, int l_rows, int l_cols, int lld_lhs, int lld_rhs) {
...
template <typename T>
void hip_copy_aux_full(T *lhs_dev, T *rhs_dev, int *l_rows_in, int *l_cols_in, int *lld_lhs_in, int *lld_rhs_in, int *debug_in, hipStream_t my_stream){
  int l_rows = *l_rows_in;
...
#ifdef WITH_GPU_STREAMS
  hip_copy_aux_full_kernel<<<blocks,threadsPerBlock,0,my_stream>>>(lhs_dev, rhs_dev, l_rows, l_cols, lld_lhs, lld_rhs);
#else
  hip_copy_aux_full_kernel<<<blocks,threadsPerBlock>>>            (lhs_dev, rhs_dev, l_rows, l_cols, lld_lhs, lld_rhs);
#endif
  ...
  if (debug)
    ...
    hipError_t cuerr = hipGetLastError();
    if (cuerr != hipSuccess){
      printf("Error in executing hip_copy_aux_full: %s\n",hipGetErrorString(cuerr));
    }
  }
}

extern "C" void hip_copy_aux_full_FromC(char dataType, intptr_t lhs_dev, intptr_t rhs_dev,
                                         int *l_rows_in, int *l_cols_in, int *lld_lhs_in, int *lld_rhs_in, int *debug_in, hipStream_t my_stream){
  if (dataType=='D') hip_copy_aux_full<double>((double *) lhs_dev, (double *) rhs_dev, l_rows_in, l_cols_in, lld_lhs_in, lld_rhs_in, debug_in, my_stream);
  if (dataType=='S') hip_copy_aux_full<float> ((float  *) lhs_dev, (float  *) rhs_dev, l_rows_in, l_cols_in, lld_lhs_in, lld_rhs_in, debug_in, my_stream);
  if (dataType=='Z') hip_copy_aux_full<hipDoubleComplex>((hipDoubleComplex *) lhs_dev, (hipDoubleComplex *) rhs_dev, l_rows_in, l_cols_in, lld_lhs_in, lld_rhs_in, debug_in, my_stream);
  if (dataType=='C') hip_copy_aux_full<hipFloatComplex> ((hipFloatComplex  *) lhs_dev, (hipFloatComplex  *) rhs_dev, l_rows_in, l_cols_in, lld_lhs_in, lld_rhs_in, debug_in, my_stream);
}

//________________________________________________________________

template <typename T>
__global__ void hip_copy_and_set_zeros_aux_full_kernel(T *a_dev, T *aux_mat_full_dev, int l_rows, int l_cols, int nblk_mult) {
...
template <typename T>
void hip_copy_and_set_zeros_aux_full(T *mat_dev, T *aux_mat_full_dev, int *l_rows_in, int *l_cols_in, int *nblk_mult_in, int *debug_in, hipStream_t my_stream){
  int l_rows = *l_rows_in;
  int l_cols = *l_cols_in;
  int nblk_mult = *nblk_mult_in;
  int debug = *debug_in;
...
#ifdef WITH_GPU_STREAMS
  hip_copy_and_set_zeros_aux_full_kernel<<<blocks,threadsPerBlock,0,my_stream>>>(mat_dev, aux_mat_full_dev, l_rows, l_cols, nblk_mult);
#else
  hip_copy_and_set_zeros_aux_full_kernel<<<blocks,threadsPerBlock>>>(mat_dev, aux_mat_full_dev, l_rows, l_cols, nblk_mult);
#endif
  ...
  if (debug)
    ...
    hipError_t cuerr = hipGetLastError();
    if (cuerr != hipSuccess){
      printf("Error in executing hip_copy_and_set_zeros_aux_full: %s\n",hipGetErrorString(cuerr));
    }
  }
}

extern "C" void hip_copy_and_set_zeros_aux_full_FromC(char dataType, intptr_t mat_dev, intptr_t aux_mat_full_dev,
                                                       int *l_rows_in, int *l_cols_in, int *nblk_mult_in, int *debug_in, hipStream_t my_stream){
  if (dataType=='D') hip_copy_and_set_zeros_aux_full<double>((double *) mat_dev, (double *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, debug_in, my_stream);
  if (dataType=='S') hip_copy_and_set_zeros_aux_full<float> ((float  *) mat_dev, (float  *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, debug_in, my_stream);
  if (dataType=='Z') hip_copy_and_set_zeros_aux_full<hipDoubleComplex>((hipDoubleComplex *) mat_dev, (hipDoubleComplex *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, debug_in, my_stream);
  if (dataType=='C') hip_copy_and_set_zeros_aux_full<hipFloatComplex> ((hipFloatComplex  *) mat_dev, (hipFloatComplex  *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, debug_in, my_stream);
}
...
template <typename T>
__global__ void hip_copy_and_set_zeros_aux_a_full_kernel(T *a_dev, T *aux_a_full_dev, int l_rows, int l_cols, int nblk_mult_cols,
...
template <typename T>
void hip_copy_and_set_zeros_aux_a_full(T *mat_dev, T *aux_mat_full_dev, int *l_rows_in, int *l_cols_in, int *nblk_mult_cols_in, 
...
#ifdef WITH_GPU_STREAMS
  hip_copy_and_set_zeros_aux_a_full_kernel<<<blocks,threadsPerBlock,0,my_stream>>>(mat_dev, aux_mat_full_dev, l_rows, l_cols, nblk_mult_cols,
                                                                                    nblk, np_bc_fine, np_cols_fine, np_cols);
#else
  hip_copy_and_set_zeros_aux_a_full_kernel<<<blocks,threadsPerBlock>>>            (mat_dev, aux_mat_full_dev, l_rows, l_cols, nblk_mult_cols,
                                                                                    nblk, np_bc_fine, np_cols_fine, np_cols);
#endif
...
extern "C" void hip_copy_and_set_zeros_aux_a_full_FromC(char dataType, intptr_t mat_dev, intptr_t aux_mat_full_dev,
  ...
  if (dataType=='D') hip_copy_and_set_zeros_aux_a_full<double>((double *) mat_dev, (double *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_cols_in, nblk_in, np_bc_fine_in, np_cols_fine_in, np_cols_in, debug_in, my_stream);
  if (dataType=='S') hip_copy_and_set_zeros_aux_a_full<float> ((float  *) mat_dev, (float  *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_cols_in, nblk_in, np_bc_fine_in, np_cols_fine_in, np_cols_in, debug_in, my_stream);
  if (dataType=='Z') hip_copy_and_set_zeros_aux_a_full<hipDoubleComplex>((hipDoubleComplex *) mat_dev, (hipDoubleComplex *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_cols_in, nblk_in, np_bc_fine_in, np_cols_fine_in, np_cols_in, debug_in, my_stream);
  if (dataType=='C') hip_copy_and_set_zeros_aux_a_full<hipFloatComplex> ((hipFloatComplex  *) mat_dev, (hipFloatComplex  *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_cols_in, nblk_in, np_bc_fine_in, np_cols_fine_in, np_cols_in, debug_in, my_stream);
}

//________________________________________________________________

template <typename T>
__global__ void hip_copy_and_set_zeros_aux_b_full_kernel(T *b_dev, T *aux_b_full_dev, int l_rows, int l_cols, int nblk_mult, 
...
template <typename T>
void hip_copy_and_set_zeros_aux_b_full(T *mat_dev, T *aux_mat_full_dev, int *l_rows_in, int *l_cols_in, int *nblk_mult_in, 
...
#ifdef WITH_GPU_STREAMS
  hip_copy_and_set_zeros_aux_b_full_kernel<<<blocks,threadsPerBlock,0,my_stream>>>(mat_dev, aux_mat_full_dev, l_rows, l_cols, nblk_mult, 
                                                                                    nblk_mult_rows, nblk, np_fine, np_rows_fine, np_rows);
#else
  hip_copy_and_set_zeros_aux_b_full_kernel<<<blocks,threadsPerBlock>>>            (mat_dev, aux_mat_full_dev, l_rows, l_cols, nblk_mult, 
                                                                                    nblk_mult_rows, nblk, np_fine, np_rows_fine, np_rows);
#endif
    ...
    if (debug)
      ...
      hipError_t cuerr = hipGetLastError();
      if (cuerr != hipSuccess){
        printf("Error in executing hip_copy_and_set_zeros_aux_b_full: %s\n",hipGetErrorString(cuerr));
      }
    }
  }

extern "C" void hip_copy_and_set_zeros_aux_b_full_FromC(char dataType, intptr_t mat_dev, intptr_t aux_mat_full_dev,
                                                       int *l_rows_in, int *l_cols_in, int *nblk_mult_in, 
                                                       int *nblk_mult_rows_in, int *nblk_in, int *np_fine_in, int *np_rows_fine_in, int *np_rows_in,
                                                       int *SM_count_in, int *debug_in, hipStream_t my_stream){
  if (dataType=='D') hip_copy_and_set_zeros_aux_b_full<double>((double *) mat_dev, (double *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, nblk_mult_rows_in, nblk_in, np_fine_in, np_rows_fine_in, np_rows_in, SM_count_in, debug_in, my_stream);
  if (dataType=='S') hip_copy_and_set_zeros_aux_b_full<float> ((float  *) mat_dev, (float  *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, nblk_mult_rows_in, nblk_in, np_fine_in, np_rows_fine_in, np_rows_in, SM_count_in, debug_in, my_stream);
  if (dataType=='Z') hip_copy_and_set_zeros_aux_b_full<hipDoubleComplex>((hipDoubleComplex *) mat_dev, (hipDoubleComplex *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, nblk_mult_rows_in, nblk_in, np_fine_in, np_rows_fine_in, np_rows_in, SM_count_in, debug_in, my_stream);
  if (dataType=='C') hip_copy_and_set_zeros_aux_b_full<hipFloatComplex> ((hipFloatComplex  *) mat_dev, (hipFloatComplex  *) aux_mat_full_dev, l_rows_in, l_cols_in, nblk_mult_in, nblk_mult_rows_in, nblk_in, np_fine_in, np_rows_fine_in, np_rows_in, SM_count_in, debug_in, my_stream);
}

//________________________________________________________________

template <typename T>
__global__ void hip_ccl_copy_buf_send_kernel(T *a_dev, T *buf_send_dev, int l_rows, int l_cols, int lld_buf, int nblk,
...
template <typename T>
void hip_ccl_copy_buf_send(T *a_dev, T *buf_send_dev, int *l_rows_in, int *l_cols_in, int *lld_buf_in, int *nblk_in,
...
#ifdef WITH_GPU_STREAMS
  hip_ccl_copy_buf_send_kernel<<<blocks,threadsPerBlock,0,my_stream>>>(a_dev, buf_send_dev, l_rows, l_cols, lld_buf, nblk,
                                                                        i_block_loc_fine_max, j_block_loc_fine_max, np_fine, np_bc_fine, 
                                                                        np_rows_fine, np_cols_fine, np_rows, np_cols);
#else
  hip_ccl_copy_buf_send_kernel<<<blocks,threadsPerBlock>>>(a_dev, buf_send_dev, l_rows, l_cols, lld_buf, nblk,
                                                            i_block_loc_fine_max, j_block_loc_fine_max, np_fine, np_bc_fine, 
                                                            np_rows_fine, np_cols_fine, np_rows, np_cols);
#endif
  ...
  if (debug)
    ...
    hipError_t cuerr = hipGetLastError();
    if (cuerr != hipSuccess){
      printf("Error in executing hip_ccl_copy_buf_send: %s\n",hipGetErrorString(cuerr));
    }
  }
}

extern "C" void hip_ccl_copy_buf_send_FromC(char dataType, intptr_t a_dev, intptr_t buf_send_dev, 
                                             ...
                                             int *np_rows_fine_in, int *np_cols_fine_in, int *np_rows_in, int *np_cols_in, int *SM_count_in, int *debug_in, hipStream_t my_stream){
  if (dataType=='D') hip_ccl_copy_buf_send<double>((double *) a_dev, (double *) buf_send_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
  ...
  if (dataType=='S') hip_ccl_copy_buf_send<float> ((float  *) a_dev, (float  *) buf_send_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
                                                    i_block_loc_fine_in, j_block_loc_fine_in, np_fine_in, np_bc_fine_in, 
                                                    np_rows_fine_in, np_cols_fine_in, np_rows_in, np_cols_in, SM_count_in, debug_in, my_stream);
  if (dataType=='Z') hip_ccl_copy_buf_send<hipDoubleComplex>((hipDoubleComplex *) a_dev, (hipDoubleComplex *) buf_send_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
                                                    i_block_loc_fine_in, j_block_loc_fine_in, np_fine_in, np_bc_fine_in, 
                                                    np_rows_fine_in, np_cols_fine_in, np_rows_in, np_cols_in, SM_count_in, debug_in, my_stream);
  if (dataType=='C') hip_ccl_copy_buf_send<hipFloatComplex> ((hipFloatComplex  *) a_dev, (hipFloatComplex  *) buf_send_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
                                                    i_block_loc_fine_in, j_block_loc_fine_in, np_fine_in, np_bc_fine_in,
                                                    np_rows_fine_in, np_cols_fine_in, np_rows_in, np_cols_in, SM_count_in, debug_in, my_stream);
}
...
template <typename T>
__global__ void hip_ccl_copy_buf_recv_kernel(T *at_col_dev, T *buf_recv_dev, int l_rows, int l_cols, int lld_buf, int nblk,
                                              int i_block_loc_fine_max, int j_block_loc_fine_max, int np_fine, int np_bc_fine, 
                                              int np_rows_fine, int np_cols_fine, int np_rows, int np_cols) {
...
template <typename T>
void hip_ccl_copy_buf_recv(T *at_col_dev, T *buf_recv_dev, int *l_rows_in, int *l_cols_in, int *lld_buf_in, int *nblk_in,
                            int *i_block_loc_fine_max_in, int *j_block_loc_fine_max_in, int *np_fine_in, int *np_bc_fine_in, 
                            int *np_rows_fine_in, int *np_cols_fine_in, int *np_rows_in, int *np_cols_in, int *SM_count_in, int *debug_in, hipStream_t my_stream){
...
#ifdef WITH_GPU_STREAMS
  hip_ccl_copy_buf_recv_kernel<<<blocks,threadsPerBlock,0,my_stream>>>(at_col_dev, buf_recv_dev, l_rows, l_cols, lld_buf, nblk,
                                                                       i_block_loc_fine_max, j_block_loc_fine_max, np_fine, np_bc_fine, 
                                                                       np_rows_fine, np_cols_fine, np_rows, np_cols);
#else
  hip_ccl_copy_buf_recv_kernel<<<blocks,threadsPerBlock>>>(at_col_dev, buf_recv_dev, l_rows, l_cols, lld_buf, nblk,
                                                           i_block_loc_fine_max, j_block_loc_fine_max, np_fine, np_bc_fine, 
                                                           np_rows_fine, np_cols_fine, np_rows, np_cols);
#endif
    ...
    if (debug)
      ...
      hipError_t cuerr = hipGetLastError();
      if (cuerr != hipSuccess){
        printf("Error in executing hip_ccl_copy_buf_recv: %s\n",hipGetErrorString(cuerr));
      }
    }
  }

extern "C" void hip_ccl_copy_buf_recv_FromC(char dataType, intptr_t at_col_dev, intptr_t buf_recv_dev, 
                                             int *l_rows_in, int *l_cols_in, int *lld_buf_in, int *nblk_in,
                                             int *i_block_loc_fine_max_in, int *j_block_loc_fine_max_in, int *np_fine_in, int *np_bc_fine_in, 
                                             int *np_rows_fine_in, int *np_cols_fine_in, int *np_rows_in, int *np_cols_in, int *SM_count_in, int *debug_in, hipStream_t my_stream){
  if (dataType=='D') hip_ccl_copy_buf_recv<double>((double *) at_col_dev, (double *) buf_recv_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
                                                    i_block_loc_fine_max_in, j_block_loc_fine_max_in, np_fine_in, np_bc_fine_in, 
                                                    np_rows_fine_in, np_cols_fine_in, np_rows_in, np_cols_in, SM_count_in, debug_in, my_stream);
  if (dataType=='S') hip_ccl_copy_buf_recv<float> ((float  *) at_col_dev, (float  *) buf_recv_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
                                                    i_block_loc_fine_max_in, j_block_loc_fine_max_in, np_fine_in, np_bc_fine_in, 
                                                    np_rows_fine_in, np_cols_fine_in, np_rows_in, np_cols_in, SM_count_in, debug_in, my_stream);
  if (dataType=='Z') hip_ccl_copy_buf_recv<hipDoubleComplex>((hipDoubleComplex *) at_col_dev, (hipDoubleComplex *) buf_recv_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
                                                    i_block_loc_fine_max_in, j_block_loc_fine_max_in, np_fine_in, np_bc_fine_in, 
                                                    np_rows_fine_in, np_cols_fine_in, np_rows_in, np_cols_in, SM_count_in, debug_in, my_stream);
  if (dataType=='C') hip_ccl_copy_buf_recv<hipFloatComplex> ((hipFloatComplex  *) at_col_dev, (hipFloatComplex  *) buf_recv_dev, l_rows_in, l_cols_in, lld_buf_in, nblk_in,
                                                    i_block_loc_fine_max_in, j_block_loc_fine_max_in, np_fine_in, np_bc_fine_in,
                                                    np_rows_fine_in, np_cols_fine_in, np_rows_in, np_cols_in, SM_count_in, debug_in, my_stream);
}
...
template <typename T>
__global__ void hip_copy_and_set_zeros_aux_ab_full_tn_kernel(T *a_dev, T *b_dev, T *aux_a_full_dev, T *aux_b_full_dev,
...
template <typename T>
__global__ void hip_copy_and_set_zeros_aux_ab_full_nt_kernel(T *a_dev, T *b_dev, T *aux_a_full_dev, T *aux_b_full_dev,
                                                              int l_rows, int l_cols, int nblk_mult_max, int nblk_mult, int nblk,
                                                              int np_ab_fine, int np_rows, int my_prow,
                                                              int np_t_fine , int np_cols, int my_pcol,
                                                              int np_dirs_fine){
...
template <typename T>
void hip_copy_and_set_zeros_aux_ab_full_tn_nt(int *a_transoposed_in, T *a_dev, T *b_dev, T *aux_a_full_dev, T *aux_b_full_dev,
    ...
    if (a_transoposed)
      {
#ifdef WITH_GPU_STREAMS
      hip_copy_and_set_zeros_aux_ab_full_tn_kernel<<<blocksPerGrid, threadsPerBlock, 0, my_stream>>>(
          ...
          np_dirs_fine);
#else
      hip_copy_and_set_zeros_aux_ab_full_tn_kernel<<<blocksPerGrid, threadsPerBlock>>>(
          a_dev, b_dev, aux_a_full_dev, aux_b_full_dev,
          l_rows, l_cols, nblk_mult_max, nblk_mult, nblk,
          np_ab_fine, np_rows, my_prow,
          np_t_fine, np_cols, my_pcol,
      ...
      {
#ifdef WITH_GPU_STREAMS
      hip_copy_and_set_zeros_aux_ab_full_nt_kernel<<<blocksPerGrid, threadsPerBlock, 0, my_stream>>>(
          a_dev, b_dev, aux_a_full_dev, aux_b_full_dev,
          l_rows, l_cols, nblk_mult_max, nblk_mult, nblk,
          np_ab_fine, np_rows, my_prow,
          np_t_fine, np_cols, my_pcol,
          np_dirs_fine);
#else
      hip_copy_and_set_zeros_aux_ab_full_nt_kernel<<<blocksPerGrid, threadsPerBlock>>>(
          a_dev, b_dev, aux_a_full_dev, aux_b_full_dev,
          l_rows, l_cols, nblk_mult_max, nblk_mult, nblk,
          np_ab_fine, np_rows, my_prow,
          np_t_fine, np_cols, my_pcol,
    ...
    if (debug)
    {
        ...
        if (cuerr != hipSuccess)
        {
            printf("Error in executing hip_copy_and_set_zeros_aux_ab_full_tn: %s\n", hipGetErrorString(cuerr));
        }
    }
}

extern "C" void hip_copy_and_set_zeros_aux_ab_full_tn_nt_FromC(char dataType, int *a_transoposed_in, intptr_t a_dev, intptr_t b_dev, intptr_t aux_a_full_dev, intptr_t aux_b_full_dev,
                                                             int *l_rows_in, int *l_cols_in, int *nblk_mult_max_in, int *nblk_mult_in, int *nblk_in,
                                                             int *np_ab_fine_in, int *np_rows_in, int *my_prow_in,
                                                             int *np_t_fine_in , int *np_cols_in, int *my_pcol_in,
                                                             int *np_dirs_fine_in,
                                                             int *SM_count_in, int *debug_in, hipStream_t my_stream){
  if (dataType == 'D') hip_copy_and_set_zeros_aux_ab_full_tn_nt<double>(a_transoposed_in, (double *)a_dev, (double *)b_dev, (double *)aux_a_full_dev, (double *)aux_b_full_dev,
  ...
  if (dataType == 'S') hip_copy_and_set_zeros_aux_ab_full_tn_nt<float>(a_transoposed_in, (float *)a_dev, (float *)b_dev, (float *)aux_a_full_dev, (float *)aux_b_full_dev,
                                                      l_rows_in, l_cols_in, nblk_mult_max_in, nblk_mult_in, nblk_in,
                                                      np_ab_fine_in, np_rows_in, my_prow_in,
                                                      np_t_fine_in , np_cols_in, my_pcol_in,
                                                      np_dirs_fine_in,
                                                      SM_count_in, debug_in, my_stream);
  if (dataType == 'Z') hip_copy_and_set_zeros_aux_ab_full_tn_nt<hipDoubleComplex>(a_transoposed_in, (hipDoubleComplex *)a_dev, (hipDoubleComplex *)b_dev, (hipDoubleComplex *)aux_a_full_dev, (hipDoubleComplex *)aux_b_full_dev,
                                                                l_rows_in, l_cols_in, nblk_mult_max_in, nblk_mult_in, nblk_in,
                                                                np_ab_fine_in, np_rows_in, my_prow_in,
                                                                np_t_fine_in , np_cols_in, my_pcol_in,
                                                                np_dirs_fine_in,
                                                                SM_count_in, debug_in, my_stream);
  if (dataType == 'C') hip_copy_and_set_zeros_aux_ab_full_tn_nt<hipFloatComplex>(a_transoposed_in, (hipFloatComplex *)a_dev, (hipFloatComplex *)b_dev, (hipFloatComplex *)aux_a_full_dev, (hipFloatComplex *)aux_b_full_dev,
                                                               l_rows_in, l_cols_in, nblk_mult_max_in, nblk_mult_in, nblk_in,
                                                               np_ab_fine_in, np_rows_in, my_prow_in,
                                                               np_t_fine_in , np_cols_in, my_pcol_in,
                                                               np_dirs_fine_in,
...
template <typename T>
__global__ void hip_update_c_tn_nt_kernel(int a_transposed, T *c_dev, T *tmp1_full_dev, T beta,
                                          int l_rows, int l_cols, int nblk_mult_max, int nblk_mult, int nblk,
...
template <typename T>
void hip_update_c_tn_nt(int *a_transposed_in, 
    ...
    if constexpr (std::is_same_v<T, hipDoubleComplex>) {
        beta = make_hipDoubleComplex(static_cast<double>(*beta_int_in), 0.0);
    } else if constexpr (std::is_same_v<T, hipFloatComplex>) {
        beta = make_hipFloatComplex(static_cast<float>(*beta_int_in), 0.0);
    } else {
...
#ifdef WITH_GPU_STREAMS
    hip_update_c_tn_nt_kernel<T><<<blocksPerGrid, threadsPerBlock, 0, my_stream>>>(
        ...
        debug);
#else
    hip_update_c_tn_nt_kernel<T><<<blocksPerGrid, threadsPerBlock>>>(
        a_transposed, c_dev, tmp1_full_dev, beta,
        l_rows, l_cols, nblk_mult_max, nblk_mult, nblk,
    ...
    if (debug) {
        ...
        hipError_t cuerr = hipGetLastError();
        if (cuerr != hipSuccess) {
            printf("Error in executing hip_update_c_tn_nt_kernel: %s\n", hipGetErrorString(cuerr));
        }
    }
}

extern "C" void hip_update_c_tn_nt_FromC(char dataType,
                                          int *a_transposed_in, 
  ...
  if (dataType == 'D') hip_update_c_tn_nt<double>(a_transposed_in, 
                                (double *)c_dev, (double *)tmp1_full_dev, beta_int_in,
                                l_rows_in, l_cols_in, nblk_mult_max_in, nblk_mult_in, nblk_in,
                                np_rows_in, np_cols_in, np_dirs_fine_in,
                                np_dirs_t_in, my_pdir_t_in, np_fine_in,
                                SM_count_in, debug_in, my_stream);
  if (dataType == 'S') hip_update_c_tn_nt<float>(a_transposed_in, 
                                (float *)c_dev, (float *)tmp1_full_dev, beta_int_in,
                                l_rows_in, l_cols_in, nblk_mult_max_in, nblk_mult_in, nblk_in,
                                np_rows_in, np_cols_in, np_dirs_fine_in,
                                np_dirs_t_in, my_pdir_t_in, np_fine_in,
                                SM_count_in, debug_in, my_stream);
  if (dataType == 'Z') hip_update_c_tn_nt<hipDoubleComplex>(a_transposed_in, 
                                          (hipDoubleComplex *)c_dev, (hipDoubleComplex *)tmp1_full_dev, beta_int_in,
                                          l_rows_in, l_cols_in, nblk_mult_max_in, nblk_mult_in, nblk_in,
                                          np_rows_in, np_cols_in, np_dirs_fine_in,
                                          np_dirs_t_in, my_pdir_t_in, np_fine_in,
                                          SM_count_in, debug_in, my_stream);
  else if (dataType == 'C') hip_update_c_tn_nt<hipFloatComplex>(a_transposed_in, 
                                        (hipFloatComplex *)c_dev, (hipFloatComplex *)tmp1_full_dev, beta_int_in,
                                        l_rows_in, l_cols_in, nblk_mult_max_in, nblk_mult_in, nblk_in,
                                        np_rows_in, np_cols_in, np_dirs_fine_in,
                                        np_dirs_t_in, my_pdir_t_in, np_fine_in,
                                        SM_count_in, debug_in, my_stream);
