...
void s_Cannons_Mult2(float* L, float* U, int np_rows, int np_cols, int my_prow, int my_pcol, int* a_desc, float *Res, MPI_Comm row_comm, MPI_Comm col_comm)
{
   ...
   float *Buf_to_send_L, *Buf_to_receive_L, *Buf_to_send_U, *Buf_to_receive_U, *U_local_start_curr, *CopyFrom, *CopyTo;
   ...
   float *L_local_start, *Buf_pos, *U_local_start, *float_ptr, *Res_ptr, *Buf_L;
...
///////////////////////////////////////////////////////////////// Start of algorithm ///////////////////////////////////////////////////////////////////////////////////////////////
   ...
   Buf_to_send_L = malloc(ratio*Size_U_stored*sizeof(float));
   Buf_to_receive_L = malloc(ratio*Size_U_stored*sizeof(float));
   Buf_to_send_U = malloc(Size_U_stored*sizeof(float));
   Buf_to_receive_U = malloc(Size_U_stored*sizeof(float));
   if(ratio != 1)
      Buf_L = malloc(Size_U_stored*sizeof(float));   // in this case we will receive data into initial buffer and after place block-columns to the needed positions of buffer for calculation
   ...
   for(i = 0; i < num_of_iters; i++)       // loop over my block-columns
   {
      ...
      if((rows_in_block > 0)&&(cols_in_block > 0))
      {
         ...
         slacpy_("A", &rows_in_block, &cols_in_block, L_local_start, &na_rows, Buf_pos, &rows_in_block);     // copy lower part of block-column in the buffer with LDA = length of the lower part of block-column 
   ...
   *Buf_pos = (float)cols_in_buffer_L_my_initial; // write number of the columns at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_L, Size_send_L, MPI_FLOAT, where_to_send_L, 0, Buf_L, Size_U_stored, MPI_FLOAT, from_where_to_receive_L, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_FLOAT, &Size_receive_L_now);
            ...
            {
               rows_in_buffer_L_now = na_rows - ceil((float)(((float)from_where_to_receive_L - (float)my_prow)/(float)np_rows))*nblk; // some of the block-rows have been skipped
            }
         ...
         intNumber = ceil((float)cols_in_buffer_L_now/(float)nblk);  // how many block-columns I have received on this iteration
      ...
      else    // if grid is square
      {
         if(my_prow > 0)
         {
            MPI_Sendrecv(Buf_to_send_L, Size_send_L, MPI_FLOAT, where_to_send_L, 0, Buf_to_receive_L, Size_U_stored, MPI_FLOAT, from_where_to_receive_L, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_FLOAT, &Size_receive_L);
            ...
            {
               rows_in_buffer_L = na_rows - ceil((float)(((float)from_where_to_receive_L - (float)my_prow)/(float)np_rows))*nblk; // some of the block-rows have been skipped
            }
         }
   ...
   *Buf_pos = (float)rows_in_buffer; // write number of the rows at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   {   
      ...
      MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_FLOAT, where_to_send_U, 0, Buf_to_receive_U, Size_U_stored, MPI_FLOAT, from_where_to_receive_U, 0, col_comm, &status); 
   ...
   for(j = 1; j < np_rows; j++)
   {
      ...
      MPI_Isend(Buf_to_send_L, Size_send_L, MPI_FLOAT, where_to_send_L, 0, row_comm, &request_L_Send); 
      MPI_Irecv(Buf_to_receive_L, ratio*Size_U_stored, MPI_FLOAT, from_where_to_receive_L, 0, row_comm, &request_L_Recv);
      ...
      { 
         ...
         if ((rows_in_block > 0)&&(cols_in_block > 0))
         {
            ...
            for (ii = 0; ii < ceil((float)rows_in_block_U/(float)nblk); ii++)
            {
               ...
               if((j == 1)&&(ii == 0))
                  sgemm_("N", "N", &rows_in_block, &cols_in_block, &rows_in_block_U_curr, &done, L_local_start, &LDA_L, U_local_start_curr, &rows_in_block_U, &dzero, Res_ptr, &na_rows); 
               else 
                  sgemm_("N", "N", &rows_in_block, &cols_in_block, &rows_in_block_U_curr, &done, L_local_start, &LDA_L, U_local_start_curr, &rows_in_block_U, &done, Res_ptr, &na_rows);
      ...
      MPI_Get_count(&status, MPI_FLOAT, &Size_receive_L); // find out how many elements I have received 
...
set_up_blacsgrid_f1(my_mpi_comm_world, &my_blacs_ctxt, &np_rows, &np_cols, &nprow, &npcol, &my_prow, &my_pcol);
   ...
   startTime = MPI_Wtime();
   s_Cannons_Mult2(a, b, np_rows, np_cols, my_prow, my_pcol, a_desc, c, mpi_comm_cols, mpi_comm_rows);
   MPI_Barrier(MPI_COMM_WORLD);
   endTime = MPI_Wtime();
   ...
   ///// test ELPA ///////////////////////////////////////////////////////////////////////////////////////////
   pstran_(&na, &na, &done, a_copy, &one, &one, a_desc, &dzero, a_t, &one, &one, a_desc);
   MPI_Barrier(MPI_COMM_WORLD);
   startTime = MPI_Wtime();
   elpa_mult_at_b_real_single('U', 'U', na, na, b, na_rows, na_cols, a_t, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, work, na_rows, na_cols);   // work has upper part of b(H)*A(H)
   MPI_Barrier(MPI_COMM_WORLD);
   endTime = MPI_Wtime();
   localTime = endTime - startTime; 
   MPI_Reduce(&localTime, &avTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   ...
   pstran_(&na, &na, &done, work, &one, &one, a_desc, &dzero, c2, &one, &one, a_desc);   // c2 has lower part of A*b
   ...
   if(myid == 0)
      printf("Cannon's algorithm. 0 proc in: %lf, average over procs = %lf, max = %lf\n", localTime, avTime, maxTime);
   
   ///// test ELPA ///////////////////////////////////////////////////////////////////////////////////////////
   pstran_(&na, &na, &done, a_copy, &one, &one, a_desc, &dzero, a_t, &one, &one, a_desc);
   MPI_Barrier(MPI_COMM_WORLD);
   startTime = MPI_Wtime();
   elpa_mult_at_b_real_single('U', 'U', na, na, b, na_rows, na_cols, a_t, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, work, na_rows, na_cols);
   MPI_Barrier(MPI_COMM_WORLD);
   endTime = MPI_Wtime();
   localTime = endTime - startTime; 
   MPI_Reduce(&localTime, &avTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   ...
   pstran_(&na, &na, &done, work, &one, &one, a_desc, &dzero, c2, &one, &one, a_desc);
