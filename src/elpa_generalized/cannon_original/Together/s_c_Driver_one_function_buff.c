...
void pcelset_(float complex*, int*, int*, int*, float complex*);
void pchemm_(char*, char*, int*, int*, float complex*, float complex*, int*, int*, int*, float complex*, int*, int*, int*, float complex*, float complex*, int*, int*, int*);
void pcpotrf_(char*, int*, float complex*, int*, int*, int*, int*);
void pchegst_(int*, char*, int*, float complex*, int*, int*, int*, float complex*, int*, int*, int*, float complex*, float complex*, int*, int*);
void descinit_(int*, int*, int*, int*, int*, int*, int*, int*, int*, int*);
int numroc_(int*, int*, int*, int*, int*);
void set_up_blacsgrid_f1(int, int*, int*, int*, int*, int*, int*, int*);
void pctrtrs_(char*, char*, char*, int*, int*, float complex*, int*, int*, int*, float complex*, int*, int*, int*, int*);
void pcheevx_(char*, char*, char*, int*, float complex*, int*, int*, int*, int*, int*, int*, int*, int*, int*, float complex*, float complex*, int*, int*, int*, float complex*, int*, int*, int*, int*);
...
void s_c_Cannons_Reduction(float complex* A, float complex* U, int np_rows, int np_cols, int my_prow, int my_pcol, int* a_desc, float complex *Res, int ToStore, MPI_Comm row_comm, MPI_Comm col_comm)
{
   // Input matrices: 
      // - A: full matrix
   ...
   float complex *Buf_to_send_A, *Buf_to_receive_A, *Buf_to_send_U, *Buf_to_receive_U, *float_ptr, *Buf_A, *Buf_pos, *U_local_start, *Res_ptr, *M, *M_T, *A_local_start, *U_local_start_curr, *U_stored, *CopyTo, *CopyFrom, *U_to_calc;
...
//////////////////////////////////////////// Start of algorithm //////////////////////////////////////////////////////////////////////////////
  ...
  if(na%nblk == 0)
   ...
   if(ratio != 1)
      Buf_A = malloc(Buf_cols*Buf_rows*sizeof(float complex));   // in this case we will receive data into initial buffer and after place block-columns to the needed positions of buffer for calculation
   ...
   if(ratio != 1)
      clacpy_("A", &na_rows, &na_cols, A, &na_rows, Buf_to_send_A, &na_rows);   // copy my buffer to send
   Size_receive_A = 0; 
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_A, na_cols*na_rows, MPI_COMPLEX, where_to_send_A, 0, Buf_A, na_rows*Buf_cols, MPI_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_A_now);
         ...
         {
            clacpy_("A", &na_rows, &na_cols, A, &na_rows, Buf_to_send_A, &na_rows);   // copy my buffer to send
            MPI_Sendrecv(Buf_to_send_A, na_cols*na_rows, MPI_COMPLEX, where_to_send_A, 0, Buf_to_receive_A, na_rows*Buf_cols, MPI_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
         ...
         {
            clacpy_("A", &na_rows, &na_cols, A, &na_rows, Buf_to_receive_A, &na_rows);   // copy A to the received buffer if I do not need to send
   ...
   {      
      ...
      if((rows_in_block > 0)&&(cols_in_block > 0))
      {
         ...
         clacpy_("A", &rows_in_block, &cols_in_block, float_ptr, &na_rows, Buf_pos, &rows_in_block);     // copy upper part of block-column in the buffer with LDA = length of the upper part of block-column 
   ...
   *Buf_pos = (float complex)rows_in_buffer; // write number of the rows at the end of the buffer; we will need this for further multiplications on the other processors
   ...
   {   
      ...
      MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_COMPLEX, where_to_send_U, 0, Buf_to_receive_U, Buf_rows*na_cols, MPI_COMPLEX, from_where_to_receive_U, 0, col_comm, &status); 
      MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_U); // find out how many elements I have received 
   }
   ...
   for(j = 1; j < np_rows; j++)
   {
      ...
      MPI_Isend(Buf_to_send_A, Size_send_A*na_rows, MPI_COMPLEX, where_to_send_A, 0, row_comm, &request_A_Send); 
      MPI_Irecv(Buf_to_receive_A, Buf_cols*na_rows*ratio, MPI_COMPLEX, from_where_to_receive_A, 0, row_comm, &request_A_Recv);
         
      ///// shift for U /////////////////////////////////////////////
      Size_send_U = Size_receive_U; 
      MPI_Isend(Buf_to_send_U, Size_send_U, MPI_COMPLEX, where_to_send_U, 0, col_comm, &request_U_Send); 
      MPI_Irecv(Buf_to_receive_U, Buf_rows*na_cols, MPI_COMPLEX, from_where_to_receive_U, 0, col_comm, &request_U_Recv); 
      ...
      MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_A); // find out how many elements I have received 
   ...
   pctranc_(&na, &na, &done, M, &one, &one, a_desc, &dzero, M_T, &one, &one, a_desc);     // now M_T has lower part of U(-H)*A 
   ...
   for(i = 0; i < num_of_iters; i++)       // loop over my block-columns
   {
      ...
      if((rows_in_block > 0)&&(cols_in_block > 0))
      {
         ...
         clacpy_("A", &rows_in_block, &cols_in_block, A_local_start, &na_rows, Buf_pos, &rows_in_block);     // copy lower part of block-column in the buffer with LDA = length of the lower part of block-column 
   ...
   *Buf_pos = (float complex)cols_in_buffer_A_my_initial; // write number of the columns at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_A, Size_send_A, MPI_COMPLEX, where_to_send_A, 0, Buf_A, Size_U_stored, MPI_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_A_now);
      ...
      else    // if grid is square
      {
         if(my_prow > 0)
         {
            MPI_Sendrecv(Buf_to_send_A, Size_send_A, MPI_COMPLEX, where_to_send_A, 0, Buf_to_receive_A, Size_U_stored, MPI_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_A);
   ...
   for(j = 1; j < np_rows; j++)
   {
      ...
      MPI_Isend(Buf_to_send_A, Size_send_A, MPI_COMPLEX, where_to_send_A, 0, row_comm, &request_A_Send); 
      MPI_Irecv(Buf_to_receive_A, ratio*Size_U_stored, MPI_COMPLEX, from_where_to_receive_A, 0, row_comm, &request_A_Recv);
      ...
      if (j > ToStore)
      {
         if(j > ToStore + 1)
         {
            ...
            MPI_Isend(U_to_calc, Size_send_U, MPI_COMPLEX, where_to_send_U, 0, col_comm, &request_U_Send);
         MPI_Irecv(Buf_to_receive_U, Size_U_stored, MPI_COMPLEX, from_where_to_receive_U, 0, col_comm, &request_U_Recv);
      }
...
void s_c_Cannons_triang_rectangular(float complex* U, float complex* B, int np_rows, int np_cols, int my_prow, int my_pcol, int* U_desc, int*b_desc, float complex *Res, MPI_Comm row_comm, MPI_Comm col_comm)
{
   ...
   float complex *U_local_start, *Buf_pos, *B_local_start, *float_ptr, *CopyTo, *CopyFrom;
   ...
   if(ratio != 1)
      Buf_U = malloc(Size_U_stored*sizeof(float complex));   // in this case we will receive data into initial buffer and after place block-rows to the needed positions of buffer for calculation
   ...
   *Buf_pos = (float complex)cols_in_buffer_U_my_initial; // write number of the columns at the end of the buffer; we will need this for furhter multiplications on the other processors
   Buf_pos = Buf_pos + 1; 
   *Buf_pos = (float complex)rows_in_buffer_U_my_initial; // write number of the rows at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_COMPLEX, where_to_send_U, 0, Buf_U, Size_U_stored, MPI_COMPLEX, from_where_to_receive_U, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_U_now);
      ...
      else    // if grid is square
      {
         if(my_prow > 0)
         {
            MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_COMPLEX, where_to_send_U, 0, Buf_to_receive_U, Size_U_stored, MPI_COMPLEX, from_where_to_receive_U, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_U);
   ...
   if(my_pcol > 0)
   {
      ...
      if(where_to_send_B != my_prow)                  // for the rectangular proc grids it may be possible that I need to "send to myself"; if it is not the case, then I send
      {
         ...
         MPI_Sendrecv(Buf_to_send_B, nb_cols*na_rows, MPI_COMPLEX, where_to_send_B, 0, Buf_to_receive_B, nb_cols*Buf_rows, MPI_COMPLEX, from_where_to_receive_B, 0, col_comm, &status); 
         MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_B); // find out how many elements I have received 
         Size_receive_B = Size_receive_B/nb_cols;    // how many rows I have received
      }
      else
      {
         clacpy_("A", &na_rows, &nb_cols, B, &na_rows, Buf_to_receive_B, &na_rows); // else I copy data like I have "received" it
         Size_receive_B = na_rows;
      }
   }
   else
   {
      clacpy_("A", &na_rows, &nb_cols, B, &na_rows, Buf_to_receive_B, &na_rows);        // if I am in the 0 proc row, I need not to send; so copy data like I have "received" it
      Size_receive_B = na_rows; 
   }   
   ...
   for(i = 1; i < np_rows; i++)
   {
      ...
      float_ptr = Buf_to_send_B; 
      Buf_to_send_B = Buf_to_receive_B; 
      Buf_to_receive_B = float_ptr;
            
      Size_send_U = Size_receive_U;
      Size_send_B = Size_receive_B;                   
        
      ///// shift for U ////////////////////////////////////////////////////////////
      MPI_Isend(Buf_to_send_U, Size_send_U, MPI_COMPLEX, where_to_send_U, 0, row_comm, &request_U_Send); 
      MPI_Irecv(Buf_to_receive_U, ratio*Size_U_stored, MPI_COMPLEX, from_where_to_receive_U, 0, row_comm, &request_U_Recv);
         
      ///// shift for B /////////////////////////////////////////////      
      MPI_Isend(Buf_to_send_B, Size_send_B*nb_cols, MPI_COMPLEX, where_to_send_B, 0, col_comm, &request_B_Send); 
      MPI_Irecv(Buf_to_receive_B, Buf_rows*nb_cols, MPI_COMPLEX, from_where_to_receive_B, 0, col_comm, &request_B_Recv);
...
int main(int argc, char** argv) {
   ...
   float complex *a, *b, *EigenVectors, *a_copy, *b_copy, *c, *AUinv, *EigVectors_gen, *EigValues, *work_find;
   float* EigValues_elpa;
   int *a_desc, *b_desc, *AUinv_desc, *c_desc, *EigenVectors_desc; 
      
   double startTime, endTime, localTime, AverageTime, MaxTime, start_in, end_in, time_invert, time_mult_from_left, time_mult_from_left2, time_mult_1, time_mult_2;
   ...
   int THIS_COMPLEX_ELPA_KERNEL_API, success; 
   float complex value; 
   ...
   float complex *Ax, *Bx, *lambdaBx, *lambda_Matr;
...
set_up_blacsgrid_f1(my_mpi_comm_world, &my_blacs_ctxt, &np_rows, &np_cols, &nprow, &npcol, &my_prow, &my_pcol);
   ...
   /////////////////////////memory allocation area//////////////////////////////////////////////////////////////
   a  = malloc(na_rows*na_cols*sizeof(float complex));
   b  = malloc(na_rows*na_cols*sizeof(float complex));
   EigValues_elpa = malloc(na*sizeof(float)); 
   EigValues = malloc(na*sizeof(float complex));
   EigenVectors = malloc(na_rows*na_cols*sizeof(float complex));
   a_copy = malloc(na_rows*na_cols*sizeof(float complex));
   b_copy = malloc(na_rows*na_cols*sizeof(float complex));
   c = malloc(na_rows*na_cols*sizeof(float complex));
   work_find = malloc(Lwork_find*sizeof(float complex));
   AUinv = malloc(na_rows*na_cols*sizeof(float complex));
   int* Iwork = malloc(Liwork*sizeof(int));
   Ax = malloc(na_rows*LocC*sizeof(float complex));
   Bx = malloc(na_rows*LocC*sizeof(float complex));
   lambdaBx = malloc(na_rows*LocC*sizeof(float complex));
   lambda_Matr = malloc(LocR_1*LocC*sizeof(float complex));
   EigVectors_gen = malloc(na_rows*na_cols*sizeof(float complex));
   ...
   for(i = 0; i < na_rows; i++)
      for(j = 0; j < na_cols; j++)
      {
         ...
         if(i_global != j_global)
            if(i_global > j_global)
               a[i + j*na_rows] = (float)cos(i_global)*cos(j_global) + (float)sin(i_global)*sin(j_global) + (float)cos(i_global)*cos(j_global)*I;
            else
               a[i + j*na_rows] = (float)cos(i_global)*cos(j_global) + (float)sin(i_global)*sin(j_global) - (float)cos(i_global)*cos(j_global)*I;
         else
            a[i + j*na_rows] = (float)cos(i_global)*cos(j_global) + (float)sin(i_global)*sin(j_global) + (float)(i_global + j_global)/na;
         if(i_global != j_global)
         {
            if(i_global > j_global)
               b[i + j*na_rows] = (float)sin(i_global)*sin(j_global);
            else
               b[i + j*na_rows] = (float)sin(i_global)*sin(j_global);
         }
         else
            b[i + j*na_rows] = (float)sin(i_global)*(float)sin(j_global) + 1;
      }
   ...
   for(rep = 0; rep < 2; rep++)
   {
      ...
      startTime = MPI_Wtime();
      elpa_cholesky_complex_single(na, b, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, 1);   // now b = U
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      start_in = MPI_Wtime();
      elpa_invert_trm_complex_single(na, b, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, 1);    // now b = U(-1)  
      MPI_Barrier(MPI_COMM_WORLD);
      end_in = MPI_Wtime();
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      s_c_Cannons_Reduction(a, b, np_rows, np_cols, my_prow, my_pcol, a_desc, AUinv, BuffLevelInt, mpi_comm_cols, mpi_comm_rows);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
...
////////////////////////////////////////////////////////////////////// Solution area ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
      ...
      startTime = MPI_Wtime();
      int useGPU = 0;
      success = elpa_solve_evp_complex_2stage_single_precision(na, na, AUinv, na_rows, EigValues_elpa, EigenVectors, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, my_mpi_comm_world, THIS_COMPLEX_ELPA_KERNEL_API, useGPU);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
      MPI_Reduce(&localTime, &AverageTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      for (i = 1; i <= nev; i++)
      {
         ...
         pcelset_(lambda_Matr, &i, &i, lambda_Matr_desc, &value);
      }
     
      /////////////////////////////////////////////////////////////////////////////////////////// back transform /////////////////////////////////////////////////////////////////////// 
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      s_c_Cannons_triang_rectangular(b, EigenVectors, np_rows, np_cols, my_prow, my_pcol, b_desc, EigenVectors_desc1, EigVectors_gen, mpi_comm_cols, mpi_comm_rows);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      ...
      pchemm_("L", "L", &na, &nev, &done, a_copy, &one, &one, a_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc1, &dzero, Ax, &one, &one, Ax_desc);
      pchemm_("L", "L", &na, &nev, &done, b_copy, &one, &one, b_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc1, &dzero, Bx, &one, &one, Bx_desc);
      pchemm_("R", "L", &na, &nev, &done, lambda_Matr, &one, &one, lambda_Matr_desc, Bx, &one, &one, Bx_desc, &dzero, lambdaBx, &one, &one, lambdaBx_desc);
      
      diff = 0;
      for (i = 0; i < na_rows; i++)
         for (j = 0; j < LocC; j++)
            diff = diff + cabsf(Ax[i + j*na_rows] - lambdaBx[i + j*na_rows]);  
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   for(rep = 0; rep < 2; rep++)
   {
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      elpa_cholesky_complex_single(na, b, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, 1);         // Upper part; Lower is 0
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
      MPI_Reduce(&localTime, &AverageTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      elpa_mult_ah_b_complex_single('U', 'L', na, na, b, na_rows, na_cols, a, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, c, na_rows, na_cols);  // now c = U(-H)*A
      pctranc_(&na, &na, &done, c, &one, &one, c_desc, &dzero, AUinv, &one, &one, AUinv_desc);  //AUinv = A*U(-1)
      elpa_mult_ah_b_complex_single('U', 'U', na, na, b, na_rows, na_cols, AUinv, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, c, na_rows, na_cols);  // now c = U(-H)*A*U(-1)
      pctranc_(&na, &na, &done, c, &one, &one, a_desc, &dzero, AUinv, &one, &one, AUinv_desc);
      pclacpy_("L", &na, &na, AUinv, &one, &one, AUinv_desc, c, &one, &one, a_desc);
      MPI_Barrier(MPI_COMM_WORLD);   
      endTime = MPI_Wtime();
      reduce_time = endTime-startTime; 
      ...
      startTime = MPI_Wtime();
      int useGPU = 0;
      success = elpa_solve_evp_complex_2stage_single_precision(na, nev, c, na_rows, EigValues_elpa, EigenVectors, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, my_mpi_comm_world, THIS_COMPLEX_ELPA_KERNEL_API, useGPU);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
...
////////////////////////////////////////////////////////////////// back transform //////////////////////////////////////////////////////////////////////////
      MPI_Barrier(MPI_COMM_WORLD);
      start_in = MPI_Wtime();
      pctranc_(&na, &na, &done, b, &one, &one, b_desc, &dzero, AUinv, &one, &one, AUinv_desc);
      elpa_mult_ah_b_complex_single('L', 'F', na, nev, AUinv, na_rows, na_cols, EigenVectors, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, EigVectors_gen, na_rows, na_cols);
      MPI_Barrier(MPI_COMM_WORLD);
      end_in = MPI_Wtime();
...
//////////////////////////////////////// Check the results //////////////////////////////////////////////////
      ...
      pchemm_("L", "L", &na, &nev, &done, a_copy, &one, &one, a_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc, &dzero, Ax, &one, &one, Ax_desc);
      pchemm_("L", "L", &na, &nev, &done, b_copy, &one, &one, b_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc, &dzero, Bx, &one, &one, Bx_desc);
      pchemm_("R", "L", &na, &nev, &done, lambda_Matr, &one, &one, lambda_Matr_desc, Bx, &one, &one, Bx_desc, &dzero, lambdaBx, &one, &one, lambdaBx_desc);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   double complex* work1 = malloc(Lwork*sizeof(double complex));
   for(rep = 0; rep < 2; rep++)
   {
      if (myid == 0)
      {
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      pcpotrf_("L", &na, b, &one, &one, b_desc, &info);           // rewrites only lower triang part of b
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
      MPI_Reduce(&localTime, &AverageTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      pchegst_(&IBYTYPE, "L", &na, a, &one, &one, a_desc, b, &one, &one, b_desc, &Scale, work1, &Lwork, &info);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      reduce_time = endTime-startTime; 
      MPI_Reduce(&reduce_time, &reduce_av, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      startTime = MPI_Wtime();
      if(nprocs < 8000)
         pcheevx_("V", "I", "L", &na, a, &one, &one, a_desc, &na, &na, &one, &nev, &new_f, &nev_f, EigValues, EigenVectors, &one, &one, EigenVectors_desc, work_find, &Lwork_find, Iwork, &Liwork, &info);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime;
...
////////////////////////////////////////////////////////////////////// back transform //////////////////////////////////////////////////////////////////////////////
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      pctrtrs_("L", "T", "N", &na, &nev, b, &one, &one, b_desc, EigenVectors, &one, &one, EigenVectors_desc, &info);  // now EigenVectors = L(-H)*x
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      back_transform_time = endTime - startTime; 
      MPI_Reduce(&back_transform_time, &back_average, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      pchemm_("L", "L", &na, &nev, &done, a_copy, &one, &one, a_copy_desc, EigenVectors, &one, &one, EigenVectors_desc, &dzero, Ax, &one, &one, Ax_desc);
      pchemm_("L", "L", &na, &nev, &done, b_copy, &one, &one, b_copy_desc, EigenVectors, &one, &one, EigenVectors_desc, &dzero, Bx, &one, &one, Bx_desc);
      pchemm_("R", "L", &na, &nev, &done, lambda_Matr, &one, &one, lambda_Matr_desc, Bx, &one, &one, Bx_desc, &dzero, lambdaBx, &one, &one, lambdaBx_desc);
      
      diff = 0;
      for (i = 0; i < na_rows; i++)
         for (j = 0; j < LocC; j++)
            diff = diff + cabs(Ax[i + j*na_rows] - lambdaBx[i + j*na_rows]);   
