...
!        !$  max_threads = omp_get_max_threads()
    ...
    if(l_nev==0) then
        ...
        stripe_width = 256
        max_threads  = l_nev/stripe_width
...
!       print *, "posting receive for l_nev*nblk = ", l_nev * nblk
    ...
    print*, 'Request buffer allocation'
...
!    print *, "Max_blk_size = ", max_blk_size
    ...
    print*, 'Buffer allocation completed'
...
contains
   ...
   subroutine pack_row_device(row, n)
    ...
    subroutine compute_hh_trafo(off, ncols, istripe)
        integer off, ncols, istripe, j
        real*8 w(nbw,2), ttt
        if(ncols <= 0) return
        ttt = mpi_wtime()

!a_host=a
!        call my_single_hh_trafo(a_host(1,1,istripe,1),bcast_buffer(1,1), nbw, stripe_width,off,a_off,ncols)
!a=a_host
bcast_buffer_dev=bcast_buffer
        call my_single_hh_trafo_dev<<<1,32>>>(a(1,1,istripe,1),bcast_buffer_dev(1,1), nbw, stripe_width,off,a_off,ncols)

        kernel_flops = kernel_flops + 4*int(stripe_width,8)*int(ncols,8)*int(nbw,8)
...
attributes(global) subroutine my_single_hh_trafo_dev(q, hh, nb, nq,off,a_off,ncols)
    ...
    integer,value:: nb, nq, ldq, off,a_off,ncols
    real*8:: q(nq, *), hh(*)

    integer:: i,j,k,qoff,hoff
    real*8:: v
    ...
    do j = ncols, 1, -1
       ...
       ! q = q - v * hh**T
       q(i,qoff+1) = q(i,qoff+1) - v
       do k=2,nb
          q(i,qoff+k) = q(i,qoff+k) - v * hh(hoff+k)
       end do
    end do
