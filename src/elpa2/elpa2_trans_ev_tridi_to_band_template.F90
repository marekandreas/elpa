...
#include "../general/precision_kinds.F90"
  ...
  if (useGPU) then
    ...
    check_alloc_cuda("trans_ev_tridi_to_band: aIntern_dev", successGPU)

    successGPU = gpu_memset(aIntern_dev , 0, num)
    check_memset_cuda("trans_ev_tridi_to_band: aIntern_dev", successGPU)
    ...
    check_host_alloc_cuda("trans_ev_tridi_to_band: row_group_host", successGPU)
    ...
    check_alloc_cuda("trans_ev_tridi_to_band: row_group_dev", successGPU)

    successGPU = gpu_memset(row_group_dev , 0, num)
    check_memset_cuda("trans_ev_tridi_to_band: row_group_dev", successGPU)
...
!                row(1:l_nev) = row(1:l_nev)
      ...
      if (useGPU) then
        ...
        check_host_register_cuda("trans_ev_tridi_to_band: top_border_send_buffer", successGPU)

        successGPU = gpu_host_register(int(loc(top_border_recv_buffer),kind=c_intptr_t), &
                      stripe_width*nbw* stripe_count * size_of_datatype,&
                      gpuHostRegisterDefault)
        check_host_register_cuda("trans_ev_tridi_to_band: top_border_recv_buffer", successGPU)

        successGPU = gpu_host_register(int(loc(bottom_border_send_buffer),kind=c_intptr_t), &
                      stripe_width*nbw* stripe_count * size_of_datatype,&
                      gpuHostRegisterDefault)
        check_host_register_cuda("trans_ev_tridi_to_band: bottom_border_send_buffer", successGPU)

        successGPU = gpu_host_register(int(loc(bottom_border_recv_buffer),kind=c_intptr_t), &
                      stripe_width*nbw* stripe_count * size_of_datatype,&
                      gpuHostRegisterDefault)
        check_host_register_cuda("trans_ev_tridi_to_band: bottom_border_recv_buffer", successGPU)
      endif
      ...
      if (useGPU) then
        ...
        check_host_alloc_cuda("trans_ev_tridi_to_band: bcast_buffer_host", successGPU)
      ...
      if (useGPU) then
        ...
        check_alloc_cuda("trans_ev_tridi_to_band: bcast_buffer_dev", successGPU)

        successGPU = gpu_memset( bcast_buffer_dev, 0, num)
        check_memset_cuda("trans_ev_tridi_to_band: bcast_buffer_dev", successGPU)

        num =  (max_blk_size)* size_of_datatype
        successGPU = gpu_malloc( hh_tau_dev, num)
        check_alloc_cuda("trans_ev_tridi_to_band: hh_tau_dev", successGPU)

        successGPU = gpu_memset( hh_tau_dev, 0, num)
        check_memset_cuda("trans_ev_tridi_to_band: hh_tau_dev", successGPU)
      endif ! useGPU
...
!            bottom_border_recv_buffer(1:nbw*stripe_width,1,i) = top_border_send_buffer(1:nbw*stripe_width,1,i)
        ...
        if (current_local_n > 1) then
          ...
          if (useGPU) then
            ...
            check_memcpy_cuda("trans_ev_tridi_to_band: bcast_buffer -> bcast_buffer_dev", successGPU)
            ...
            if (current_n_end < current_n) then
              ...
              if (useGPU) then
                ...
                successGPU =  gpu_memcpy( aIntern_dev + dev_offset , &
                                           ...
                                           gpuMemcpyHostToDevice)
                check_memcpy_cuda("trans_ev_tridi_to_band: bottom_border_recv_buffer -> aIntern_dev", successGPU)
              else
                call obj%timer%start("OpenMP parallel" // PRECISION_SUFFIX)
                !$omp parallel do &
                !$omp default(none) &
           ...
           if (top_msg_length>0) then
             ...
             if (useGPU) then
               ...
               successGPU =  gpu_memcpy( aIntern_dev+dev_offset , int(loc(top_border_recv_buffer(1,i)),kind=c_intptr_t),  &
                                           ...
                                           gpuMemcpyHostToDevice)
                check_memcpy_cuda("trans_ev_tridi_to_band: top_border_recv_buffer -> aIntern_dev", successGPU)
             else
               call obj%timer%start("OpenMP parallel" // PRECISION_SUFFIX)
           ...
           if (bottom_msg_length>0) then
             ...
             if (useGPU) then
               ...
               successGPU =  gpu_memcpy( int(loc(bottom_border_send_buffer(1,i)),kind=c_intptr_t), aIntern_dev + dev_offset, &
                ...
                check_memcpy_cuda("trans_ev_tridi_to_band: aIntern_dev -> bottom_border_send_buffer", successGPU)
      ...
      if (my_prow > 0) then
        ...
        if (useGPU) then
          ...
          successGPU =  gpu_memcpy(int(loc(top_border_send_buffer(:,i)),kind=c_intptr_t), aIntern_dev + dev_offset, &
                                    stripe_width*nbw * size_of_datatype, &
                                    gpuMemcpyDeviceToHost)
          check_memcpy_cuda("trans_ev_tridi_to_band: aIntern_dev -> top_border_send_buffer", successGPU)
...
!                result_buffer(1:l_nev*nblk,1,nbuf) =  result_buffer(1:l_nev*nblk,1,nbuf)
      ...
      if (useGPU) then
        do i = 1, stripe_count
          ...
          do j = top_msg_length+1, top_msg_length+next_local_n, chunk
            ...
            check_memcpy_cuda("trans_ev_tridi_to_band: aIntern_dev -> aIntern_dev", successGPU)
          enddo
        end do
      else
        call obj%timer%start("OpenMP parallel" // PRECISION_SUFFIX)
  ...
  if (useGPU) then
    ...
    check_host_dealloc_cuda("trans_ev_tridi_to_band: bcast_buffer_host", successGPU)
  else
  ...
  if (useGPU) then
    ...
    check_dealloc_cuda("trans_ev_tridi_to_band: aIntern_dev", successGPU)

    successGPU = gpu_free(hh_tau_dev)
    check_dealloc_cuda("trans_ev_tridi_to_band: hh_tau_dev", successGPU)

    nullify(row_group)

    successGPU = gpu_free_host(row_group_host)
    check_host_dealloc_cuda("trans_ev_tridi_to_band: row_group_host", successGPU)

    successGPU = gpu_free(row_group_dev)
    check_dealloc_cuda("trans_ev_tridi_to_band: row_group_dev", successGPU)

    successGPU =  gpu_free(bcast_buffer_dev)
    check_dealloc_cuda("trans_ev_tridi_to_band: bcast_buffer_dev", successGPU)

    successGPU = gpu_host_unregister(int(loc(top_border_send_buffer),kind=c_intptr_t))
    check_host_unregister_cuda("trans_ev_tridi_to_band: top_border_send_buffer", successGPU)

    successGPU = gpu_host_unregister(int(loc(top_border_recv_buffer),kind=c_intptr_t))
    check_host_unregister_cuda("trans_ev_tridi_to_band: top_border_recv_buffer", successGPU)

    successGPU = gpu_host_unregister(int(loc(bottom_border_send_buffer),kind=c_intptr_t))
    check_host_unregister_cuda("trans_ev_tridi_to_band: bottom_border_send_buffer", successGPU)

    successGPU = gpu_host_unregister(int(loc(bottom_border_recv_buffer),kind=c_intptr_t))
    check_host_unregister_cuda("trans_ev_tridi_to_band: bottom_border_recv_buffer", successGPU)
  endif ! useGPU
