...
void d_c_Cannons_Reduction(double complex* A, double complex* U, int np_rows, int np_cols, int my_prow, int my_pcol, int* a_desc, double complex *Res, int ToStore, MPI_Comm row_comm, MPI_Comm col_comm)
{
   ...
   double complex *Buf_to_send_A, *Buf_to_receive_A, *Buf_to_send_U, *Buf_to_receive_U, *double_ptr, *Buf_A, *Buf_pos, *U_local_start, *Res_ptr, *M, *M_T, *A_local_start, *U_local_start_curr, *U_stored, *CopyTo, *CopyFrom, *U_to_calc;
...
//////////////////////////////////////////// Start of algorithm //////////////////////////////////////////////////////////////////////////////
  ...
  if(na%nblk == 0)
   ...
   if(ratio != 1)
      Buf_A = malloc(Buf_cols*Buf_rows*sizeof(double complex));   // in this case we will receive data into initial buffer and after place block-columns to the needed positions of buffer for calculation
   ...
   if(ratio != 1)
      zlacpy_("A", &na_rows, &na_cols, A, &na_rows, Buf_to_send_A, &na_rows);   // copy my buffer to send
   Size_receive_A = 0; 
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_A, na_cols*na_rows, MPI_DOUBLE_COMPLEX, where_to_send_A, 0, Buf_A, na_rows*Buf_cols, MPI_DOUBLE_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_A_now);
         ...
         {
            zlacpy_("A", &na_rows, &na_cols, A, &na_rows, Buf_to_send_A, &na_rows);   // copy my buffer to send
            MPI_Sendrecv(Buf_to_send_A, na_cols*na_rows, MPI_DOUBLE_COMPLEX, where_to_send_A, 0, Buf_to_receive_A, na_rows*Buf_cols, MPI_DOUBLE_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_A);
            Size_receive_A = Size_receive_A/na_rows;       // how many columns of A I have received
         }
         else
         {
            zlacpy_("A", &na_rows, &na_cols, A, &na_rows, Buf_to_receive_A, &na_rows);   // copy A to the received buffer if I do not need to send
   ...
   {      
      ...
      if((rows_in_block > 0)&&(cols_in_block > 0))
      {
         ...
         zlacpy_("A", &rows_in_block, &cols_in_block, double_ptr, &na_rows, Buf_pos, &rows_in_block);     // copy upper part of block-column in the buffer with LDA = length of the upper part of block-column 
   ...
   *Buf_pos = (double complex)rows_in_buffer; // write number of the rows at the end of the buffer; we will need this for further multiplications on the other processors
   ...
   {   
      ...
      MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_DOUBLE_COMPLEX, where_to_send_U, 0, Buf_to_receive_U, Buf_rows*na_cols, MPI_DOUBLE_COMPLEX, from_where_to_receive_U, 0, col_comm, &status); 
      MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_U); // find out how many elements I have received 
   }
   ...
   for(j = 1; j < np_rows; j++)
   {
      ...
      MPI_Isend(Buf_to_send_A, Size_send_A*na_rows, MPI_DOUBLE_COMPLEX, where_to_send_A, 0, row_comm, &request_A_Send); 
      MPI_Irecv(Buf_to_receive_A, Buf_cols*na_rows*ratio, MPI_DOUBLE_COMPLEX, from_where_to_receive_A, 0, row_comm, &request_A_Recv);
         
      ///// shift for U /////////////////////////////////////////////
      Size_send_U = Size_receive_U; 
      MPI_Isend(Buf_to_send_U, Size_send_U, MPI_DOUBLE_COMPLEX, where_to_send_U, 0, col_comm, &request_U_Send); 
      MPI_Irecv(Buf_to_receive_U, Buf_rows*na_cols, MPI_DOUBLE_COMPLEX, from_where_to_receive_U, 0, col_comm, &request_U_Recv); 
      ...
      MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_A); // find out how many elements I have received 
   ...
   pztranc_(&na, &na, &done, M, &one, &one, a_desc, &dzero, M_T, &one, &one, a_desc);     // now M_T has lower part of U(-H)*A 
   ...
   for(i = 0; i < num_of_iters; i++)       // loop over my block-columns
   {
      ...
      if((rows_in_block > 0)&&(cols_in_block > 0))
      {
         ...
         zlacpy_("A", &rows_in_block, &cols_in_block, A_local_start, &na_rows, Buf_pos, &rows_in_block);     // copy lower part of block-column in the buffer with LDA = length of the lower part of block-column 
   ...
   *Buf_pos = (double complex)cols_in_buffer_A_my_initial; // write number of the columns at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_A, Size_send_A, MPI_DOUBLE_COMPLEX, where_to_send_A, 0, Buf_A, Size_U_stored, MPI_DOUBLE_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_A_now);
      ...
      else    // if grid is square
      {
         if(my_prow > 0)
         {
            MPI_Sendrecv(Buf_to_send_A, Size_send_A, MPI_DOUBLE_COMPLEX, where_to_send_A, 0, Buf_to_receive_A, Size_U_stored, MPI_DOUBLE_COMPLEX, from_where_to_receive_A, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_A);
   ...
   for(j = 1; j < np_rows; j++)
   {
      ...
      MPI_Isend(Buf_to_send_A, Size_send_A, MPI_DOUBLE_COMPLEX, where_to_send_A, 0, row_comm, &request_A_Send); 
      MPI_Irecv(Buf_to_receive_A, ratio*Size_U_stored, MPI_DOUBLE_COMPLEX, from_where_to_receive_A, 0, row_comm, &request_A_Recv);
      ...
      if (j > ToStore)
      {
         if(j > ToStore + 1)
         {
            ...
            MPI_Isend(U_to_calc, Size_send_U, MPI_DOUBLE_COMPLEX, where_to_send_U, 0, col_comm, &request_U_Send);
         MPI_Irecv(Buf_to_receive_U, Size_U_stored, MPI_DOUBLE_COMPLEX, from_where_to_receive_U, 0, col_comm, &request_U_Recv);
      }
...
void d_c_Cannons_triang_rectangular(double complex* U, double complex* B, int np_rows, int np_cols, int my_prow, int my_pcol, int* U_desc, int*b_desc, double complex *Res, MPI_Comm row_comm, MPI_Comm col_comm)
{
   ...
   if(ratio != 1)
      Buf_U = malloc(Size_U_stored*sizeof(double complex));   // in this case we will receive data into initial buffer and after place block-rows to the needed positions of buffer for calculation
   ...
   *Buf_pos = (double complex)cols_in_buffer_U_my_initial; // write number of the columns at the end of the buffer; we will need this for furhter multiplications on the other processors
   Buf_pos = Buf_pos + 1; 
   *Buf_pos = (double complex)rows_in_buffer_U_my_initial; // write number of the rows at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_DOUBLE_COMPLEX, where_to_send_U, 0, Buf_U, Size_U_stored, MPI_DOUBLE_COMPLEX, from_where_to_receive_U, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_U_now);
      ...
      else    // if grid is square
      {
         if(my_prow > 0)
         {
            MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_DOUBLE_COMPLEX, where_to_send_U, 0, Buf_to_receive_U, Size_U_stored, MPI_DOUBLE_COMPLEX, from_where_to_receive_U, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_U);
   ...
   if(my_pcol > 0)
   {
      ...
      if(where_to_send_B != my_prow)                  // for the rectangular proc grids it may be possible that I need to "send to myself"; if it is not the case, then I send
      {
         ...
         MPI_Sendrecv(Buf_to_send_B, nb_cols*na_rows, MPI_DOUBLE_COMPLEX, where_to_send_B, 0, Buf_to_receive_B, nb_cols*Buf_rows, MPI_DOUBLE_COMPLEX, from_where_to_receive_B, 0, col_comm, &status); 
         MPI_Get_count(&status, MPI_DOUBLE_COMPLEX, &Size_receive_B); // find out how many elements I have received 
         Size_receive_B = Size_receive_B/nb_cols;    // how many rows I have received
      }
      else
      {
         zlacpy_("A", &na_rows, &nb_cols, B, &na_rows, Buf_to_receive_B, &na_rows); // else I copy data like I have "received" it
         Size_receive_B = na_rows;
      }
   }
   else
   {
      zlacpy_("A", &na_rows, &nb_cols, B, &na_rows, Buf_to_receive_B, &na_rows);        // if I am in the 0 proc row, I need not to send; so copy data like I have "received" it
      Size_receive_B = na_rows; 
   }   
   ...
   for(i = 1; i < np_rows; i++)
   {
      ...
      ///// shift for U ////////////////////////////////////////////////////////////
      MPI_Isend(Buf_to_send_U, Size_send_U, MPI_DOUBLE_COMPLEX, where_to_send_U, 0, row_comm, &request_U_Send); 
      MPI_Irecv(Buf_to_receive_U, ratio*Size_U_stored, MPI_DOUBLE_COMPLEX, from_where_to_receive_U, 0, row_comm, &request_U_Recv);
         
      ///// shift for B /////////////////////////////////////////////      
      MPI_Isend(Buf_to_send_B, Size_send_B*nb_cols, MPI_DOUBLE_COMPLEX, where_to_send_B, 0, col_comm, &request_B_Send); 
      MPI_Irecv(Buf_to_receive_B, Buf_rows*nb_cols, MPI_DOUBLE_COMPLEX, from_where_to_receive_B, 0, col_comm, &request_B_Recv);
...
int main(int argc, char** argv) {
   ...
   double complex *a, *b, *EigenVectors, *a_copy, *b_copy, *c, *AUinv, *EigVectors_gen, *EigValues, *work_find;
   double* EigValues_elpa;
   ...
   double complex *Ax, *Bx, *lambdaBx, *lambda_Matr;
...
set_up_blacsgrid_f1(my_mpi_comm_world, &my_blacs_ctxt, &np_rows, &np_cols, &nprow, &npcol, &my_prow, &my_pcol);
   ...
   /////////////////////////memory allocation area//////////////////////////////////////////////////////////////
   a  = malloc(na_rows*na_cols*sizeof(double complex));
   b  = malloc(na_rows*na_cols*sizeof(double complex));
   EigValues_elpa = malloc(na*sizeof(double)); 
   EigValues = malloc(na*sizeof(double complex));
   EigenVectors = malloc(na_rows*na_cols*sizeof(double complex));
   a_copy = malloc(na_rows*na_cols*sizeof(double complex));
   b_copy = malloc(na_rows*na_cols*sizeof(double complex));
   c = malloc(na_rows*na_cols*sizeof(double complex));
   work_find = malloc(Lwork_find*sizeof(double complex));
   AUinv = malloc(na_rows*na_cols*sizeof(double complex));
   int* Iwork = malloc(Liwork*sizeof(int));
   Ax = malloc(na_rows*LocC*sizeof(double complex));
   Bx = malloc(na_rows*LocC*sizeof(double complex));
   lambdaBx = malloc(na_rows*LocC*sizeof(double complex));
   lambda_Matr = malloc(LocR_1*LocC*sizeof(double complex));
   EigVectors_gen = malloc(na_rows*na_cols*sizeof(double complex));
   ...
   for(i = 0; i < na_rows; i++)
      for(j = 0; j < na_cols; j++)
      {
         ...
         if(i_global != j_global)
            if(i_global > j_global)
               a[i + j*na_rows] = (double)cos(i_global)*cos(j_global) + (double)sin(i_global)*sin(j_global) + (double)cos(i_global)*cos(j_global)*I;
            else
               a[i + j*na_rows] = (double)cos(i_global)*cos(j_global) + (double)sin(i_global)*sin(j_global) - (double)cos(i_global)*cos(j_global)*I;
         else
            a[i + j*na_rows] = (double)cos(i_global)*cos(j_global) + (double)sin(i_global)*sin(j_global) + (double)(i_global + j_global)/na;
         if(i_global != j_global)
         {
            if(i_global > j_global)
               b[i + j*na_rows] = (double)sin(i_global)*sin(j_global);
            else
               b[i + j*na_rows] = (double)sin(i_global)*sin(j_global);
         }
         else
            b[i + j*na_rows] = (double)sin(i_global)*(double)sin(j_global) + 1;
      }
   ...
   for(rep = 0; rep < 2; rep++)
   {
      ...
      startTime = MPI_Wtime();
      elpa_cholesky_complex_double(na, b, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, 1);   // now b = U
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      start_in = MPI_Wtime();
      elpa_invert_trm_complex_double(na, b, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, 1);    // now b = U(-1)  
      MPI_Barrier(MPI_COMM_WORLD);
      end_in = MPI_Wtime();
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      d_c_Cannons_Reduction(a, b, np_rows, np_cols, my_prow, my_pcol, a_desc, AUinv, BuffLevelInt, mpi_comm_cols, mpi_comm_rows);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
...
////////////////////////////////////////////////////////////////////// Solution area ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////// 
      ...
      startTime = MPI_Wtime();
      int useGPU = 0;
      success = elpa_solve_evp_complex_2stage_double_precision(na, na, AUinv, na_rows, EigValues_elpa, EigenVectors, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, my_mpi_comm_world, THIS_COMPLEX_ELPA_KERNEL_API, useGPU);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
      MPI_Reduce(&localTime, &AverageTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      for (i = 1; i <= nev; i++)
      {
         ...
         pzelset_(lambda_Matr, &i, &i, lambda_Matr_desc, &value);
      }
     
      /////////////////////////////////////////////////////////////////////////////////////////// back transform /////////////////////////////////////////////////////////////////////// 
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      d_c_Cannons_triang_rectangular(b, EigenVectors, np_rows, np_cols, my_prow, my_pcol, b_desc, EigenVectors_desc1, EigVectors_gen, mpi_comm_cols, mpi_comm_rows);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      ...
      pzhemm_("L", "L", &na, &nev, &done, a_copy, &one, &one, a_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc1, &dzero, Ax, &one, &one, Ax_desc);
      pzhemm_("L", "L", &na, &nev, &done, b_copy, &one, &one, b_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc1, &dzero, Bx, &one, &one, Bx_desc);
      pzhemm_("R", "L", &na, &nev, &done, lambda_Matr, &one, &one, lambda_Matr_desc, Bx, &one, &one, Bx_desc, &dzero, lambdaBx, &one, &one, lambdaBx_desc);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   for(rep = 0; rep < 2; rep++)
   {
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      elpa_cholesky_complex_double(na, b, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, 1);         // Upper part; Lower is 0
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
      MPI_Reduce(&localTime, &AverageTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      elpa_mult_ah_b_complex_double('U', 'L', na, na, b, na_rows, na_cols, a, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, c, na_rows, na_cols);  // now c = U(-H)*A
      pztranc_(&na, &na, &done, c, &one, &one, c_desc, &dzero, AUinv, &one, &one, AUinv_desc);  //AUinv = A*U(-1)
      elpa_mult_ah_b_complex_double('U', 'U', na, na, b, na_rows, na_cols, AUinv, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, c, na_rows, na_cols);  // now c = U(-H)*A*U(-1)
      pztranc_(&na, &na, &done, c, &one, &one, a_desc, &dzero, AUinv, &one, &one, AUinv_desc);
      pzlacpy_("L", &na, &na, AUinv, &one, &one, AUinv_desc, c, &one, &one, a_desc);
      MPI_Barrier(MPI_COMM_WORLD);   
      endTime = MPI_Wtime();
      reduce_time = endTime-startTime; 
      ...
      startTime = MPI_Wtime();
      int useGPU = 0;
      success = elpa_solve_evp_complex_2stage_double_precision(na, nev, c, na_rows, EigValues_elpa, EigenVectors, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, my_mpi_comm_world, THIS_COMPLEX_ELPA_KERNEL_API, useGPU);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
...
////////////////////////////////////////////////////////////////// back transform //////////////////////////////////////////////////////////////////////////
      MPI_Barrier(MPI_COMM_WORLD);
      start_in = MPI_Wtime();
      pztranc_(&na, &na, &done, b, &one, &one, b_desc, &dzero, AUinv, &one, &one, AUinv_desc);
      elpa_mult_ah_b_complex_double('L', 'F', na, nev, AUinv, na_rows, na_cols, EigenVectors, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, EigVectors_gen, na_rows, na_cols);
      MPI_Barrier(MPI_COMM_WORLD);
      end_in = MPI_Wtime();
...
//////////////////////////////////////// Check the results //////////////////////////////////////////////////
      ...
      pzhemm_("L", "L", &na, &nev, &done, a_copy, &one, &one, a_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc, &dzero, Ax, &one, &one, Ax_desc);
      pzhemm_("L", "L", &na, &nev, &done, b_copy, &one, &one, b_copy_desc, EigVectors_gen, &one, &one, EigenVectors_desc, &dzero, Bx, &one, &one, Bx_desc);
      pzhemm_("R", "L", &na, &nev, &done, lambda_Matr, &one, &one, lambda_Matr_desc, Bx, &one, &one, Bx_desc, &dzero, lambdaBx, &one, &one, lambdaBx_desc);
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   ...
   for(rep = 0; rep < 2; rep++)
   {
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      pzpotrf_("L", &na, b, &one, &one, b_desc, &info);           // rewrites only lower triang part of b
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime; 
      MPI_Reduce(&localTime, &AverageTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      pzhegst_(&IBYTYPE, "L", &na, a, &one, &one, a_desc, b, &one, &one, b_desc, &Scale, work1, &Lwork, &info);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      reduce_time = endTime-startTime; 
      MPI_Reduce(&reduce_time, &reduce_av, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      startTime = MPI_Wtime();
      if(nprocs < 8000)
         pzheevx_("V", "I", "L", &na, a, &one, &one, a_desc, &na, &na, &one, &nev, &new_f, &nev_f, EigValues, EigenVectors, &one, &one, EigenVectors_desc, work_find, &Lwork_find, Iwork, &Liwork, &info);
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      localTime = endTime - startTime;
...
////////////////////////////////////////////////////////////////////// back transform //////////////////////////////////////////////////////////////////////////////
      MPI_Barrier(MPI_COMM_WORLD);
      startTime = MPI_Wtime();
      pztrtrs_("L", "T", "N", &na, &nev, b, &one, &one, b_desc, EigenVectors, &one, &one, EigenVectors_desc, &info);  // now EigenVectors = L(-H)*x
      MPI_Barrier(MPI_COMM_WORLD);
      endTime = MPI_Wtime();
      back_transform_time = endTime - startTime; 
      MPI_Reduce(&back_transform_time, &back_average, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
      ...
      pzhemm_("L", "L", &na, &nev, &done, a_copy, &one, &one, a_copy_desc, EigenVectors, &one, &one, EigenVectors_desc, &dzero, Ax, &one, &one, Ax_desc);
      pzhemm_("L", "L", &na, &nev, &done, b_copy, &one, &one, b_copy_desc, EigenVectors, &one, &one, EigenVectors_desc, &dzero, Bx, &one, &one, Bx_desc);
      pzhemm_("R", "L", &na, &nev, &done, lambda_Matr, &one, &one, lambda_Matr_desc, Bx, &one, &one, Bx_desc, &dzero, lambdaBx, &one, &one, lambdaBx_desc);
