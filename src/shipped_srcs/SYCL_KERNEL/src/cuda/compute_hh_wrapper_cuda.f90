module compute_hh_wrapper_cuda
  ...
  subroutine compute_hh_cuda_gpu_complex(nn,nc,nbw,q,hh,tau, init_time, transfer_time, compute_time)
    ...
    ! Compute
    t_begin = omp_get_wtime()
    call compute_hh_cuda_gpu_complex_kernel(q_dev,hh_dev,tau_dev,nc,nbw,nc,nn)
    ok = cuda_device_synchronize()
    t_end = omp_get_wtime()
    compute_time = t_end - t_begin
