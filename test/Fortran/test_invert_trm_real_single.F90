...
program test_invert_trm
  ...
  use output_types
   ...
   real(kind=rk4), allocatable :: d(:), e(:), bs(:,:)
  ...
  call timer%start("program")
   ...
   call prepare_matrix_single(na, myid, sc_desc, iseed,  b, z, bs)
   bs(:,:) = b(:,:)
   ...
   success = elpa_invert_trm_real_single(na, a, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, .true.)
...
#ifdef WITH_MPI
   call psgemm("N","N", na, na, na, 1.0_rk4, as, 1, 1, sc_desc, a, 1, 1, &
               sc_desc, 0.0_rk4, tmp1, 1, 1, sc_desc)
#else
   call sgemm("N","N", na, na, na, 1.0_rk4, as, na, a, na, 0.0_rk4, tmp1, na)
#endif
   ...
   ! tmp2 = b * tmp1
#ifdef WITH_MPI
   call psgemm("N","N", na, na, na, 1.0_rk4, b, 1, 1, sc_desc, tmp1, 1, 1, &
               sc_desc, 0.0_rk4, tmp2, 1, 1, sc_desc)
#else
   call sgemm("N","N", na, na, na, 1.0_rk4, b, na, tmp1, na, 0.0_rk4, tmp2, na)
#endif
...
#ifdef WITH_MPI
   norm = pslange("M",na, na, tmp2, 1, 1, sc_desc, bs)
#else
   norm = slange("M", na, na, tmp2, na_rows, bs)
#endif
