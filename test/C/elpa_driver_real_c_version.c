...
int main(int argc, char** argv) {
   ...
   if (myid == 0) {
     ...
     printf("\n");
     printf("It will call the ELPA real solver for an\n");
  ...
  my_mpi_comm_world = 1;
   ...
   success = elpa_solve_evp_real_double(na, nev, a, na_rows, ev, z, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, my_mpi_comm_world, THIS_REAL_ELPA_KERNEL_API, useQr, "1stage");
   ...
   success = elpa_solve_evp_real_double(na, nev, a, na_rows, ev, z, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, my_mpi_comm_world, THIS_REAL_ELPA_KERNEL_API, useQr, "2stage");
   ...
   if (myid == 0) {
     printf("\n");
     printf("Entering auto-chosen ELPA real solver\n");
     printf("\n");
   }
#ifdef WITH_MPI
   mpierr = MPI_Barrier(MPI_COMM_WORLD);
   ...
   success = elpa_solve_evp_real_double(na, nev, a, na_rows, ev, z, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, my_mpi_comm_world, THIS_REAL_ELPA_KERNEL_API, useQr, "auto");
   ...
   if (myid == 0) {
     printf("\n");
     printf("Auto-chosen ELPA real solver complete\n");
     printf("\n");
   }
