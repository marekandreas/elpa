...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void s_c_Cannons_Mult2(float complex* L, float complex* U, int np_rows, int np_cols, int my_prow, int my_pcol, int* a_desc, float complex *Res, MPI_Comm row_comm, MPI_Comm col_comm)
{
   // Input matrices: 
   ...
   float complex *Buf_to_send_L, *Buf_to_receive_L, *Buf_to_send_U, *Buf_to_receive_U, *U_local_start_curr, *CopyFrom, *CopyTo;
   ...
   float complex *L_local_start, *Buf_pos, *U_local_start, *float_ptr, *Res_ptr, *Buf_L;
   ...
   Buf_to_send_L = malloc(ratio*Size_U_stored*sizeof(float complex));
   Buf_to_receive_L = malloc(ratio*Size_U_stored*sizeof(float complex));
   Buf_to_send_U = malloc(Size_U_stored*sizeof(float complex));
   Buf_to_receive_U = malloc(Size_U_stored*sizeof(float complex));
   if(ratio != 1)
      Buf_L = malloc(Size_U_stored*sizeof(float complex));   // in this case we will receive data into initial buffer and after place block-columns to the needed positions of buffer for calculation
   ...
   for(i = 0; i < num_of_iters; i++)       // loop over my block-columns
   {
      ...
      if((rows_in_block > 0)&&(cols_in_block > 0))
      {
         ...
         clacpy_("A", &rows_in_block, &cols_in_block, L_local_start, &na_rows, Buf_pos, &rows_in_block);     // copy lower part of block-column in the buffer with LDA = length of the lower part of block-column 
   ...
   *Buf_pos = (float complex)cols_in_buffer_L_my_initial; // write number of the columns at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   for(i = 0; i < ratio; i++)
   {
      ...
      if(ratio != 1)   // if grid is not square
      {
         ...
         {
            MPI_Sendrecv(Buf_to_send_L, Size_send_L, MPI_COMPLEX, where_to_send_L, 0, Buf_L, Size_U_stored, MPI_COMPLEX, from_where_to_receive_L, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_L_now);
      ...
      else    // if grid is square
      {
         if(my_prow > 0)
         {
            MPI_Sendrecv(Buf_to_send_L, Size_send_L, MPI_COMPLEX, where_to_send_L, 0, Buf_to_receive_L, Size_U_stored, MPI_COMPLEX, from_where_to_receive_L, 0, row_comm, &status);
            MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_L);
   ...
   *Buf_pos = (float complex)rows_in_buffer; // write number of the rows at the end of the buffer; we will need this for furhter multiplications on the other processors
   ...
   {   
      ...
      MPI_Sendrecv(Buf_to_send_U, Size_send_U, MPI_COMPLEX, where_to_send_U, 0, Buf_to_receive_U, Size_U_stored, MPI_COMPLEX, from_where_to_receive_U, 0, col_comm, &status); 
   ...
   for(j = 1; j < np_rows; j++)
   {
      ...
      float_ptr = Buf_to_send_L; 
      Buf_to_send_L = Buf_to_receive_L; 
      Buf_to_receive_L = float_ptr; 
      ...
      MPI_Isend(Buf_to_send_L, Size_send_L, MPI_COMPLEX, where_to_send_L, 0, row_comm, &request_L_Send); 
      MPI_Irecv(Buf_to_receive_L, ratio*Size_U_stored, MPI_COMPLEX, from_where_to_receive_L, 0, row_comm, &request_L_Recv);
      ...
      { 
         ...
         if ((rows_in_block > 0)&&(cols_in_block > 0))
         {
            ...
            for (ii = 0; ii < ceil((double)rows_in_block_U/(double)nblk); ii++)
            {
               ...
               if((j == 1)&&(ii == 0))
                  cgemm_("N", "N", &rows_in_block, &cols_in_block, &rows_in_block_U_curr, &done, L_local_start, &LDA_L, U_local_start_curr, &rows_in_block_U, &dzero, Res_ptr, &na_rows); 
               else 
                  cgemm_("N", "N", &rows_in_block, &cols_in_block, &rows_in_block_U_curr, &done, L_local_start, &LDA_L, U_local_start_curr, &rows_in_block_U, &done, Res_ptr, &na_rows);
      ...
      MPI_Get_count(&status, MPI_COMPLEX, &Size_receive_L); // find out how many elements I have received 
...
int main(int argc, char** argv) {
   ...
   float complex *a, *b,  *c, *a_copy, *b_copy, *c1, *c2, *a_t, *work;
   int *a_desc, *b_desc, *c_desc; 
      
   float complex value; 
   
   float complex done = 1.0; 
   float complex dMinusOne = -1.0; 
   int one = 1; 
   float complex dzero = 0.0; 
   int zero = 0;
...
set_up_blacsgrid_f1(my_mpi_comm_world, &my_blacs_ctxt, &np_rows, &np_cols, &nprow, &npcol, &my_prow, &my_pcol);
   ...
   c1 = malloc(na_rows*na_cols*sizeof(float complex));
   c2 = malloc(na_rows*na_cols*sizeof(float complex));
   a_t  = malloc(na_rows*na_cols*sizeof(float complex));
   work = malloc(na_cols*na_rows*sizeof(float complex));
   ...
   startTime = MPI_Wtime();
   s_c_Cannons_Mult2(a, b, np_rows, np_cols, my_prow, my_pcol, a_desc, c, mpi_comm_cols, mpi_comm_rows);
   MPI_Barrier(MPI_COMM_WORLD);
   endTime = MPI_Wtime();
   ...
   MPI_Barrier(MPI_COMM_WORLD);
   startTime = MPI_Wtime();
   pctrmm_("R", "U", "N", "N", &na, &na, &done, b_copy, &one, &one, b_desc, c1, &one, &one, c_desc);
   MPI_Barrier(MPI_COMM_WORLD);
   endTime = MPI_Wtime();
   localTime = endTime - startTime; 
   MPI_Reduce(&localTime, &avTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   ...
   ///// test ELPA ///////////////////////////////////////////////////////////////////////////////////////////
   pctranc_(&na, &na, &done, a_copy, &one, &one, a_desc, &dzero, a_t, &one, &one, a_desc);
   MPI_Barrier(MPI_COMM_WORLD);
   startTime = MPI_Wtime();
   elpa_mult_ah_b_complex_single('U', 'U', na, na, b, na_rows, na_cols, a_t, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, work, na_rows, na_cols);   // work has upper part of b(H)*A(H)
   MPI_Barrier(MPI_COMM_WORLD);
   endTime = MPI_Wtime();
   localTime = endTime - startTime; 
   MPI_Reduce(&localTime, &avTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   ...
   pctranc_(&na, &na, &done, work, &one, &one, a_desc, &dzero, c2, &one, &one, a_desc);   // c2 has lower part of A*b
   ...
   diffSum = 0; 
   for(i = 0; i < na_rows*na_cols; i++)
      diff = diff + cabs(c1[i]-c[i]);
      
   MPI_Reduce(&diff, &diffSum, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
   if(myid == 0)
      printf("Summed difference between Cannon's and PCTRMM = %.15e, average = %.15e\n", diffSum, diffSum/(na*na));
   
   diff = 0;
   diffSum = 0; 
   for(i = 0; i < na_rows*na_cols; i++)
      diff = diff + cabs(c2[i]-c[i]);
   MPI_Reduce(&diff, &diffSum, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
   if(myid == 0)
   ...
   diffSum = 0; 
   for(i = 0; i < na_rows*na_cols; i++)
      diff = diff + cabs(c2[i]-c1[i]);
   MPI_Reduce(&diff, &diffSum, 1, MPI_FLOAT, MPI_SUM, 0, MPI_COMM_WORLD);
   if(myid == 0)
      printf("Summed difference between PCTRMM and ELPA = %.15e, average = %.15e\n", diffSum, diffSum/(na*na));
   ...
   MPI_Barrier(MPI_COMM_WORLD);
   startTime = MPI_Wtime();
   elpa_mult_ah_b_complex_single('U', 'U', na, na, b, na_rows, na_cols, a_t, na_rows, na_cols, nblk, mpi_comm_rows, mpi_comm_cols, work, na_rows, na_cols);
   MPI_Barrier(MPI_COMM_WORLD);
   endTime = MPI_Wtime();
   localTime = endTime - startTime; 
   MPI_Reduce(&localTime, &avTime, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
   ...
   pctranc_(&na, &na, &done, work, &one, &one, a_desc, &dzero, c2, &one, &one, a_desc);
