...
interface
    subroutine launch_compute_hh_trafo_c_kernel(q, hh, hh_dot, hh_tau, nev, nb, ldq, off, ncols) bind(c)
        ...
        real*8, device :: q(*)
        real*8, device, intent(in) :: hh(*), hh_dot(*), hh_tau(*)
    end subroutine
end interface
...
!   nbw = (31/nblk+1)*nblk
   ...
   print *, " about to back transform tridi"
...
#include "tridiag_band_real_gpu_v1.f90"	! Use this one for the GPU

#include "ev_tridi_band_gpu_v3.f90"		! Use this one for the GPU

!#include "ev_tridi_band_gpu_v3_working_parallel_packKernels.f90"
