...
module elpa1_auxiliary
  ...
  public :: solve_tridi_double         !< Solve tridiagonal eigensystem for a double-precision matrix with divide and conquer method
  ...
  public :: cholesky_real_single       !< Cholesky factorization of a single-precision real matrix
  public :: invert_trm_real_single     !< Invert single-precision real triangular matrix
  public :: mult_at_b_real_single      !< Multiply single-precision real matrices A**T * B
  public :: solve_tridi_single         !< Solve tridiagonal eigensystem for a single-precision matrix with divide and conquer method
#endif

#ifdef WANT_SINGLE_PRECISION_COMPLEX
  public :: cholesky_complex_single    !< Cholesky factorization of a single-precision complex matrix
  public :: invert_trm_complex_single  !< Invert single-precision complex triangular matrix
  public :: mult_ah_b_complex_single   !< Multiply single-precision complex matrices A**H * B
#endif
...
!> \param succes                logical, reports success or failure
  ...
  !> \brief  invert_trm_real: Old, deprecated interface for inversion of a double-precision real upper triangular matrix
!> \details
!> \param  na                   Order of matrix
...
!> \brief  cholesky_complex: Old, deprecated interface for a Cholesky factorization of a double-precision complex hermitian matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be factorized.
!>                              Distribution is like in Scalapack.
...
!> \param succes                logical, reports success or failure
  ...
  !> \brief  invert_trm_complex:  Old, deprecated interface to invert a double-precision complex upper triangular matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be inverted
!>                              Distribution is like in Scalapack.
...
!> \param succes                logical, reports success or failure
  ...
  !> \brief  mult_at_b_real:  Old, deprecated interface for double-precision Performs C : = A**T * B
...
!> \brief  mult_ah_b_complex: Old, deprecated interfaced to Performs C : = A**H * B
!>         where   A is a square matrix (na,na) which is optionally upper or lower triangular
!>                 B is a (na,ncb) matrix
!>                 C is a (na,ncb) matrix where optionally only the upper or lower
!>                   triangle may be computed
...
!> \param ldc                   leading dimension of matrix c
  ...
  !> \brief  solve_tridi: Old, deprecated interface to solve tridiagonal eigensystem for a double-precision matrix with divide and conquer method
!> \details
!>
...
#define REAL_DATATYPE rk8
    ...
    subroutine cholesky_real_double(na, a, lda, nblk, matrixCols, mpi_comm_rows, mpi_comm_cols, &
                                            wantDebug, success)
     use elpa1_compute
     use elpa_utilities
     use elpa_mpi
      ...
      call timer%start("cholesky_real_double")
#else
      call timer%start("cholesky_real_single")
#endif
#endif
...
#ifdef HAVE_DETAILED_TIMINGS
#ifdef DOUBLE_PRECISION_REAL
      call timer%stop("cholesky_real_double")
#else
      call timer%stop("cholesky_real_single")
#endif
#endif

    end subroutine cholesky_real_double
...
!> \param succes                logical, reports success or failure
    ...
    subroutine cholesky_real_single(na, a, lda, nblk, matrixCols, mpi_comm_rows, mpi_comm_cols, &
                                            wantDebug, success)
     use elpa1_compute
     use elpa_utilities
     use elpa_mpi
    ...
    end subroutine cholesky_real_single
#endif /* WANT_SINGLE_PRECISION_REAL */

!> \brief  invert_trm_real_double: Inverts a double-precision real upper triangular matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be inverted
!>                              Distribution is like in Scalapack.
...
#define REAL_DATATYPE rk8
    ...
    subroutine invert_trm_real_double(na, a, lda, nblk, matrixCols, mpi_comm_rows, mpi_comm_cols, wantDebug, success)
       use precision
       use elpa1_compute
       use elpa_utilities
       use elpa_mpi
    ...
    end subroutine invert_trm_real_double
...
!> \brief  invert_trm_real_single: Inverts a single-precision real upper triangular matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be inverted
!>                              Distribution is like in Scalapack.
...
!> \param succes                logical, reports success or failure
    ...
    subroutine invert_trm_real_single(na, a, lda, nblk, matrixCols, mpi_comm_rows, mpi_comm_cols, wantDebug, success)
       use precision
       use elpa1_compute
       use elpa_utilities
       use elpa_mpi
    ...
    end subroutine invert_trm_real_single

#endif /* WANT_SINGLE_PRECISION_REAL */

!> \brief  cholesky_complex_double: Cholesky factorization of a double-precision complex hermitian matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be factorized.
!>                              Distribution is like in Scalapack.
...
!> \brief  cholesky_complex_single: Cholesky factorization of a single-precision complex hermitian matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be factorized.
!>                              Distribution is like in Scalapack.
...
!> \brief  invert_trm_complex_double: Inverts a double-precision complex upper triangular matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be inverted
!>                              Distribution is like in Scalapack.
...
!> \brief  invert_trm_complex_single: Inverts a single-precision complex upper triangular matrix
!> \details
!> \param  na                   Order of matrix
!> \param  a(lda,matrixCols)    Distributed matrix which should be inverted
!>                              Distribution is like in Scalapack.
...
#define REAL_DATATYPE rk8
    ...
    subroutine mult_at_b_real_double(uplo_a, uplo_c, na, ncb, a, lda, ldaCols, b, ldb, ldbCols, nblk, &
...
#ifdef HAVE_DETAILED_TIMINGS
#ifdef DOUBLE_PRECISION_REAL
      call timer%start("mult_at_b_real_double")
#else
      call timer%start("mult_at_b_real_single")
#endif
#endif
!      if (na .lt. lda) then
...
#ifdef HAVE_DETAILED_TIMINGS
#ifdef DOUBLE_PRECISION_REAL
      call timer%stop("mult_at_b_real_double")
#else
      call timer%stop("mult_at_b_real_single")
#endif

#endif

    end subroutine mult_at_b_real_double
...
!> \brief  mult_at_b_real_single: Performs C : = A**T * B
!>         where   A is a square matrix (na,na) which is optionally upper or lower triangular
!>                 B is a (na,ncb) matrix
!>                 C is a (na,ncb) matrix where optionally only the upper or lower
!>                   triangle may be computed
...
!> \param ldc                   leading dimension of matrix c
    ...
    subroutine mult_at_b_real_single(uplo_a, uplo_c, na, ncb, a, lda, ldaCols, b, ldb, ldbCols, nblk, &
                              mpi_comm_rows, mpi_comm_cols, c, ldc, ldcCols)
#ifdef HAVE_DETAILED_TIMINGS
      use timings
#endif
...
!              tmp2 = tmp1
    ...
    end subroutine mult_at_b_real_single

#endif /* WANT_SINGLE_PRECISION_REAL */

!> \brief  mult_ah_b_complex_double: Performs C : = A**H * B
!>         where   A is a square matrix (na,na) which is optionally upper or lower triangular
!>                 B is a (na,ncb) matrix
!>                 C is a (na,ncb) matrix where optionally only the upper or lower
!>                   triangle may be computed
...
!> \brief  mult_ah_b_complex_single: Performs C : = A**H * B
!>         where   A is a square matrix (na,na) which is optionally upper or lower triangular
!>                 B is a (na,ncb) matrix
!>                 C is a (na,ncb) matrix where optionally only the upper or lower
!>                   triangle may be computed
...
!> \brief  solve_tridi_double: Solve tridiagonal eigensystem for a double-precision matrix with divide and conquer method
!> \details
!>
!> \param na                    Matrix dimension
!> \param nev                   number of eigenvalues/vectors to be computed
...
!> \result success              logical, .true. on success, else .false.
    ...
    function solve_tridi_double(na, nev, d, e, q, ldq, nblk, matrixCols, mpi_comm_rows, mpi_comm_cols, wantDebug) result(success)
...
#ifdef WANT_SINGLE_PRECISION_REAL
!> \brief  solve_tridi_single: Solve tridiagonal eigensystem for a single-precision matrix with divide and conquer method
!> \details
!>
!> \param na                    Matrix dimension
!> \param nev                   number of eigenvalues/vectors to be computed
...
!> \result success              logical, .true. on success, else .false.
    ...
    function solve_tridi_single(na, nev, d, e, q, ldq, nblk, matrixCols, mpi_comm_rows, mpi_comm_cols, wantDebug) result(success)
