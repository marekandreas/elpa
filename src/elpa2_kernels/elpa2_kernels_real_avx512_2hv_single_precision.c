...
__forceinline void hh_trafo_kernel_4_AVX512_2hv_single(float* q, float* hh, int nb, int ldq, int ldh, float s);
//__forceinline void hh_trafo_kernel_4_sse_instead_of_avx512_2hv_single(float* q, float* hh, int nb, int ldq, int ldh, float s);
...
void double_hh_trafo_real_avx512_2hv_single(float* q, float* hh, int* pnb, int* pnq, int* pldq, int* pldh)
{
	...
	{
		hh_trafo_kernel_24_AVX512_2hv_single(&q[i], hh, nb, ldq, ldh, s);
	}
	if (nq == i)
	{
		return;
	...
	if (nq-i == 20)
	{
		...
		hh_trafo_kernel_4_AVX512_2hv_single(&q[i+16], hh, nb, ldq, ldh, s);
// 		hh_trafo_kernel_4_sse_instead_of_avx512_2hv_single(&q[i+8], hh, nb, ldq, ldh, s);
	...
	{
		hh_trafo_kernel_8_AVX512_2hv_single(&q[i], hh, nb, ldq, ldh, s);
		hh_trafo_kernel_4_AVX512_2hv_single(&q[i+8], hh, nb, ldq, ldh, s);
// 		hh_trafo_kernel_4_sse_instead_of_avx512_2hv_single(&q[i+8], hh, nb, ldq, ldh, s);
	}
	else if (nq-i == 8)
	{
	...
	{
		hh_trafo_kernel_4_AVX512_2hv_single(&q[i], hh, nb, ldq, ldh, s);
//		hh_trafo_kernel_4_sse_instead_of_avx512_2hv_single(&q[i], hh, nb, ldq, ldh, s);
