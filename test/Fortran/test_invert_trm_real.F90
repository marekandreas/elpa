...
program test_invert_trm
  ...
  use output_types
   ...
   real(kind=rk8), allocatable :: d(:), e(:), bs(:,:)
  ...
  call timer%start("program")
   ...
   call prepare_matrix_double(na, myid, sc_desc, iseed,  b, z, bs)
   bs(:,:) = b(:,:)

   a(:,:) = 0.0_rk8


   diagonalElement = 2.546_rk8
   ...
   success = elpa_cholesky_real_double(na, a, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, .true.)
   ...
   success = elpa_invert_trm_real_double(na, a, na_rows, nblk, na_cols, mpi_comm_rows, mpi_comm_cols, .true.)
...
#ifdef WITH_MPI
   call pdgemm("N","N", na, na, na, 1.0_rk8, as, 1, 1, sc_desc, a, 1, 1, &
               sc_desc, 0.0_rk8, tmp1, 1, 1, sc_desc)
#else
   call dgemm("N","N", na, na, na, 1.0_rk8, as, na, a, na, 0.0_rk8, tmp1, na)
#endif
   ...
   ! tmp2 = b * tmp1
#ifdef WITH_MPI
   call pdgemm("N","N", na, na, na, 1.0_rk8, b, 1, 1, sc_desc, tmp1, 1, 1, &
               sc_desc, 0.0_rk8, tmp2, 1, 1, sc_desc)
#else
   call dgemm("N","N", na, na, na, 1.0_rk8, b, na, tmp1, na, 0.0_rk8, tmp2, na)
#endif
...
#ifdef WITH_MPI
   norm = pdlange("M",na, na, tmp2, 1, 1, sc_desc, bs)
#else
   norm = dlange("M", na, na, tmp2, na_rows, bs)
#endif
